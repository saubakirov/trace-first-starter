---
name: "source-command-tfw-init"
description: "TFW Init — initialize TFW in a new project, guided by AI agent"
---

# source-command-tfw-init

Use this skill when the user asks to run the migrated source command `tfw-init`.
...
- "Which AI tool are you using? (Codex / Cursor / Antigravity / multiple)"
...
4. **Adapter files** — based on user's tool choice:
   - Codex: copy `AGENTS.md.template` → `AGENTS.md`, fill in project values.
     Copy each `.tfw/workflows/*.md` → `.Codex/commands/tfw-{name}.md` (e.g. `plan.md` → `tfw-plan.md`, etc.)
...
- [ ] **Slash commands copied** — verify adapter workflows exist:
  ...
  - Codex: `.Codex/commands/tfw-plan.md`, `tfw-handoff.md`, `tfw-review.md` (+ others)
