...
| 3 | **Debt disposed** — every REVIEW §5 row carries `paid` / `promoted` / `not material`, ruled by the **coordinator** and not by the reviewer (`conventions.md` §15), and the disposition names something that **already exists**: a phase directory, or a task directory and `status.md` created now. `→ backlog` and "someone should open a task" are not dispositions. `pending — coordinator` / `pending — owner` is legal and keeps the task open. **And the ruling's grammar holds**: a named consequence or its named absence — a bare priority is inadmissible — and `not material` says which question it answers, *not owed* or *owed and forbidden to pay* with the barring clause cited _(kept on consequence, not rate: the undisposed item is precisely what filled the registry retired at 2.1.0 — 77 of its 121 rows open, none consumed)_ | ✅/❌/⚪ | {each §5 row, its disposition, what that disposition names, and the consequence its ruling states} |
...
- [ ] Row 3: every §5 row disposed by the coordinator, each disposition naming something that exists today, and each ruling naming a consequence or its absence rather than a priority?
