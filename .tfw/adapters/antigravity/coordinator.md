...
The owner reported on 2026-09-23 that full Antigravity agent chats in the tested surface have no spawn route and must be created manually. Earlier reports mention subagents, existing-session communication and unavailable title changes; they are bounded observations, not a complete TFW trial or a claim about today's active surface. Inspect `provision · addressed send · wait/readback · title/readback` independently at the relevant gate and classify each `native`, `owner-assisted` or `unavailable`. Check title write and exact readback now; if unsupported, name that one navigation limit and continue without a success claim. Do not translate Codex or Claude mechanisms into this surface.
