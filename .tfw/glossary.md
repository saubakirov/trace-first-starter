...
## Disposition
The ruling a debt item carries before its task can close: **paid** as a phase of that task, **promoted** to a task created then and there, or **not material**, ruled on the record beside the item. Exactly three, and a disposition must name something that already exists — *"→ backlog"* and *"someone should open a task"* name nothing and are not dispositions. The reviewer proposes; the **coordinator** rules, once at the close of review. `pending — coordinator` and `pending — owner` are legal waiting states, not a fourth outcome, and each keeps the task open until it becomes one of the three. → `templates/REVIEW.md` §5, conventions.md §15
