...
Большинство проектов не умеет объяснить себя. Ход рассуждений остаётся в исчезнувшем или недоступном чате, в чьей-то памяти или на встрече без протокола. Новый участник команды или новый сеанс с ИИ видит итог, но не знает цели, ограничений, доказательств и безопасного следующего шага.

Trace-First Workflow (TFW) делает работу проверяемой и пригодной для продолжения. **TFW — методология совместной работы людей и ИИ, основанная на Философии Следа.** Цель, легитимные полномочия, суждение, приёмка, ответственность за результат и за решение об остановке работы остаются у людей; агенты выполняют ограниченные поручения. **След (Trace)** — это отобранный устойчивый во времени контекст, а не сырой протокол чата и не скрытая цепочка рассуждений. В нём сохраняются решения, результат или текущее состояние, доказательства, границы и сведения для продолжения.
...
### 🎯 Руководителям продукта, которые обеспечивают выполнение решений в нескольких командах
...
Предыдущий анализ трудно найти, между исследовательскими итерациями теряются предпосылки, а итоговый отчёт редко показывает решения, которые его сформировали. TFW сохраняет выводы каждой итерации, гипотезы и результаты их проверки, ограничения и решения для последующей проверки и консолидации.
...
В Full TFW команда `/tfw-plan` создаёт или уточняет план задачи и служит точкой входа для продолжения прерванной работы, `/tfw-handoff` выполняет утверждённый TS, а `/tfw-review` независимо проверяет завершённую работу. Остальным процессам соответствуют `/tfw-research`, `/tfw-docs`, `/tfw-knowledge`, `/tfw-release`, `/tfw-update`, `/tfw-config` и `/tfw-init`.
...
Да. TFW упорядочивает решения и преемственность, а не только программирование. Light вырос из реального нетехнического учебного сценария; тот же подход применим к исследованиям, аналитике, текстам, преподаванию, дизайну и операциям.
...
|---|---|
| **Контекст проекта можно проверить** | Цель, решения, ограничения, отвергнутые альтернативы, доказательства, состояние и долг доступны рядом с результатом. Если проект называют «осознающим себя», имеются в виду эти проверяемые возможности, а не человеческие свойства |
| **Продолжение от явной контрольной точки** | Человек или агент читает локальный `status.md`, журнал и нужные Следы, сверяет записанное состояние и продолжает с явной передачи, а не с исчезнувшего чата |
| **Знание может накапливаться** | Следы задач сохраняют сведения-кандидаты для долговременного знания; проверка и консолидация переводят подтверждённые факты в устойчивое знание, не объявляя истиной каждую заметку |
| **У людей и агентов разные обязанности** | У людей остаются цель, полномочия, суждение, приёмка, ответственность за результат и за решение об остановке работы; агенты выполняют ограниченные роли внутри утверждённого договора |
...
| Файл | Назначение |
|---|---|
| `README.md` | Практическое руководство и ссылки на метод, знания, релизы и выбранные Следы |
...
> **Реестр технического долга снят с обслуживания 2026-09-02.** Корневой `TECH_DEBT.md` запечатан
> непрочитанным в [`tasks/DEBT-SNAPSHOT.md`](tasks/DEBT-SNAPSHOT.md) и стал историей. Долг, найденный
> в ревью, записывается один раз — в самом REVIEW — и до закрытия задачи получает одно из трёх
> решений: оплатить фазой, вынести в отдельную задачу или под запись признать несущественным.
> Обязанность вести общий список снята; вести свой никому не запрещено.
...
Полный цикл Full виден в локальном состоянии задачи и файлах Следов:

```text
⬜ TODO → 📝 HL_DRAFT → 🔬 RES → 🟡 TS_DRAFT → 🟠 ONB → 🟢 RF → 🔍 REV → 📚 KNW → ✅ DONE
```

Этапы `RES` и `KNW` проходят только при выполнении соответствующих условий проекта. Статус `❌ REJECTED` сохраняет неудачную попытку и полученный опыт; `❌ BLOCKED` фиксирует тупик, не выдавая его за завершение.
...
| Правила поведения | Работать прямо, конкретно и полностью; не льстить, не оставлять заглушки и не запрашивать секреты открытым текстом | [правила](.tfw/conventions.md) |
...
| Изучить историю и доказательства | [Выбранный След исполнения](workspace/2026/TFW_20260904-113200_VBSA/HL-TFW_20260904-113200_VBSA.md) · [Проверенные знания](KNOWLEDGE.md) · [История версий](.tfw/CHANGELOG.md) |
