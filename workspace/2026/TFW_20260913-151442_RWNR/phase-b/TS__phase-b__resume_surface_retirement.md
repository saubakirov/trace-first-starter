...
- Update exactly two existing assurance modules for live absence, ten-command parity, clean installation, version-addressed owned-only retirement, history preservation, complete accounting, failure mutants, and Phase A regression.
...
The 30 VALUE paths are one connected semantic group. Five exact artifacts delete and the other 25 modify; no VALUE creation or rename is approved. The two assurance modules may change only to prove this result and retain Phase A coverage. No split is useful: deleting canonical payload without registrations, receivers, current docs, or migration proof would violate the master DoF. Phase-local TRACE never enlarges VALUE.
...
- [ ] Baseline→Candidate VALUE diff contains all and only the 30 literal VALUE paths with exactly 5 DELETE and 25 MODIFY actions; the two assurance paths are the only non-TRACE implementation changes.
...
- [ ] Candidate commit contains exactly the 30 VALUE and two ASSURANCE paths. TRACE-only ONB/evidence/RF/REVIEW/journal writes occur after it and never move Candidate.

Gate: `python -m pytest docs/scripts/test_runtime_context.py docs/scripts/test_repository_contracts.py -q`; configured `python -m pytest tools/tests/ docs/scripts/ -q --collect-only`; configured `python -m pytest tools/tests/ docs/scripts/ -q`; exact Git scope/accounting replay.
...
- [ ] Before the first destructive edit, verify Phase A terminal/producer lineage, exact Baseline objects, selector membership, no new live match, all connected receiver ownership, history before-images, and test prerequisites. Any miss records the invariant and stops with the implementation path set identical to Baseline.
...
- [ ] Any independent REVIEW rejection leaves the Candidate unlanded and local `master` at the accepted live-Resume baseline; the rejected Candidate may remain reachable only as traceable evidence.
- [ ] Any landing/integrated-check mismatch stops before G2, keeps the last accepted `master`, and returns the exact failed invariant to the Coordinator/owner.
...
- [ ] Only after Phase B independent acceptance, producer landing/reachability, and integrated checks may LEAD prepare the read-only G2 comparison of `RWNR` alone, `RWNR + TKL`, and deferral.
...
- ❌ A VALUE path outside the 30-path selector changes, one of the 30 is omitted, action counts differ from 5 DELETE + 25 MODIFY, or an unapproved implementation path enters Candidate.
...
| G2 starts from unintegrated evidence | require independent approval, reachable producers, local master landing, and integrated checks before the comparison |
