...
- Why not create a role with a “wait” prompt and send the skill later? — FRATS already rejected that as activation, and it makes the first message a non-TFW instruction. It adds a chat turn without solving the address/authority race.
- Why not treat `activation: delegated` alone as proof for any session? — It would fail to bind the newly created native unit to the authorized Coordinator and could accept a foreign or duplicate role. Research must find the smallest address-binding mechanism rather than remove the provenance check.
