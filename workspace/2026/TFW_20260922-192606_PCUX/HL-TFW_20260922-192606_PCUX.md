...
| H5 | A delegated Coordinator can create a Codex role task whose first message is only `/tfw-research <task ID>` (or the corresponding role command), while the role obtains mandate, scope, route, and evidence from task-local TFW files; no provisional chat, briefing, or owner intervention is required. The result determines whether the common activation-source rule needs a correction before this route can be claimed. | open |
