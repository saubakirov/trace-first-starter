...
### C4 — Reader coherence and independent dialogue/GATEWAY selections
...
The **current** sources are asymmetric: `tfw_state.py` accepts `gateway:{address}` with `tfw-gates-only` but rejects `owner:{human}` with `iterative`; Session identity/Plan require iterative dialogue for a GATEWAY title, and current core peer-dialogue prose requires a separate GATEWAY. The **target** HL §3 and §3.4 instead makes launch delegation, dialogue permission and owner-context topology independent selections. TS must therefore correct both directions of the coupling: a selected GATEWAY may run gates-only, and an exact owner-approved bounded peer dialogue must not acquire a mandatory GATEWAY topology merely because dialogue is `iterative`. This does not relax the dialogue grant: it still names the actual two peers, purpose, boundary, consolidator, durable output and stop condition; the independent Reviewer cannot be a participant or consolidator. Where a GATEWAY is selected, it stays separate and receives only the responsible Coordinator's consolidated return, never peer traffic. Where the owner remains the `owner_gateway`, only the Coordinator reports upward. Dialogue permission never grants role creation, execution authority or a wider send edge. The current PCUX mandate remains gates-only. Sources: `tools/tfw_state.py` `validate_status`; `.tfw/conventions.md` → `Session identity`, `Workflow activation and routing`; frozen HL §§3, 3.4, 3.6 and A5.
...
**Stage decision:** close focused Challenge. Recommend one live-current status plus immutable owner-decision provenance (Extract S1), truthful later child-address binding, a constrained root-to-phase decision reference with phase-local effective state, and separate new-launch versus same-unit-continuation checks. H6's target removes both GATEWAY/dialogue dependencies while retaining an exact bounded peer grant and independent Reviewer. Leave exact syntax/validator migration to TS; do not require another owner mandate for an unchanged scoped choice, remove existing model/effort selection, or describe untested provider mechanics as proven.
