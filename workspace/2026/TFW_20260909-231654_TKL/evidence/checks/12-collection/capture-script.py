...
(destination / "capture-script.py").write_bytes(Path(__file__).read_bytes())
command = [executable, *arguments]
started = datetime.now(timezone.utc).isoformat()
clock = time.perf_counter()
environment = os.environ.copy()
environment["PYTHONIOENCODING"] = "utf-8"
environment["TFW_ASSURANCE_BUILD_CAPTURE"] = (destination / "build").as_posix()
...
receipt = {
    ...
    "capture_script_sha256": hashlib.sha256((destination / "capture-script.py").read_bytes()).hexdigest(),
