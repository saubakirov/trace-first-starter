...
> **Mindset: Analyst.** This stage compares complete configurations before selecting a design. It distinguishes required jobs from proposed files, and analytical economy from measured behavior.
...
### C0 — Re-observe only; no new durable carrier
...
| D5 | Repair existing update/init/release/conventions/templates and required installed copies only. Add no guide, template, registry or task-local release-note carrier. |
| D6 | Reuse current semantic VALUE/TRACE/ASSURANCE definitions and connected-effect reasoning; clarify only a demonstrated ambiguity in the existing canonical section. |
| D7 | The final briefing states observed outcome, material project benefit, limits and exact next action. An interrupted run has no durable message-state memory and therefore repeats the account after re-observation. |
| D8 | Move the twelve tracked singular self-install files to their already-declared plural manifest destinations; leave unrelated singular neighbors untouched. |
| D9 | Source parity and synthetic interruption/repeat tests only; native and owner-comprehension claims remain bounded to later authorized evidence. |

### C1 — Reuse an ordinary TFW task and Git; no new artifact type
...
| D5 | Update workflow routes into the existing task lifecycle; no receipt template or global update guide is added. Existing status/journal/evidence contracts become mandatory readers for update continuation. |
| D6 | Existing semantic classification applies; the task selector names actual VALUE and necessary evidence, while unrelated TRACE remains outside the Candidate. |
| D7 | RF/final briefing carries the owner account; status/journal record whether it was prepared, while actual receipt/comprehension remains unclaimed. |
| D8 | Same plural move and neighbor-preservation rule as C0, with exact paths in the task selector. |
| D9 | Source and synthetic tests plus normal task review. Review proves artifact/trace consistency, not owner comprehension or product-native invocation. |

### C2 — Full revision-2 TS configuration
...
| D5 | Modify all 23 selected existing non-rename files, create all three selected files, and perform all twelve selected singular→plural renames. The canonical workflows retain algorithms; routers/full copies synchronize them. |
| D6 | Add PV/receipt/incidental-trace terminology and apply connected semantic-effect rules across conventions, handoff and review. |
| D7 | Generalize briefing beyond four changelog blocks; add a task-local `deliverables/RELEASE-NOTES.md` for the eventual release owner and the version-independent experience guide for future updaters. |
| D8 | Move all twelve tracked self-install files to plural roots; update Antigravity/readme evidence boundaries; preserve unrelated singular-root neighbors and legacy compatibility. |
| D9 | Use source-derived local tests, synthetic receiver/harness evidence, bounded native field evidence already authorized elsewhere, and independent review; distinguish invocation from comprehension. |

### C3 — Compact receipt configuration; merge duplicate guidance into existing authorities
...
| D5 | Canonical update/release workflows become consumers of the registry/engine; project contracts supply parameters rather than concrete procedures. |
| D6 | The engine stores effect groups and classifications as structured state. |
| D7 | Render owner accounts and release notes from registry events. |
| D8 | Treat adapter roots as versioned topology migrations represented in registry state. |
| D9 | Add schema/engine tests plus existing source, synthetic, native and owner layers. |

## Findings

### E1 — Source facts rule out file-count minimization and expose the real comparison

The source census establishes a three-column comparison: completed local 3.0.0 behavior, still-applicable historical evidence, and the unimplemented revision-2 TS. The 38 TS logical rows are proposed scope, not 38 independently demonstrated missing jobs. Conversely, C0's zero new files is not evidence of completeness.

The configurations therefore use nine required-case measures rather than a raw path score. Analytical counts below are useful only after the missing-condition check:

| Configuration | New tracked proposed entities | Mandatory persistent instance on each update | Analytical logical VALUE surface | First structural concern |
|---|---:|---:|---:|---|
...
### E2 — Exact authoritative reading paths favor one algorithm plus conditional evidence

The following paths name actual or proposed readers. A slash means the next document is read; parentheses mark conditional reads.

| Case | C0 | C1 | C2 | C3 |
|---|---|---|---|---|
| Ordinary update | installed config/version/README → immutable target `update.md` → applicable changelog/(major migration) → manifest at adapter sync → briefing | C0 path + task status/scope/journal before writes and RF after verification | C0 path + prospective `update-experience.md`; conventions/receipt template are additional planned readers whose exact activation is not fixed by the TS table | C0 path; conventions only at a disputed ownership/landing rule, receipt template only when writing the receipt |
| Repeat/recovery | current receiver/source → target `update.md` → rerun verification/message | task status/journal/evidence → current receiver/source → target workflow | current receiver/source + applicable immutable receipt/attachment → target workflow; never receipt alone | same as C2, without a second update guide |
| Legacy purpose | Git baseline or owner-supplied existing locus → current HL North Star list | task baseline/evidence attachment → current HL North Star list | content-addressed attachment → amended conventions → amended HL/judge instructions | content-addressed README attachment → existing HL's allowed README list; historical judge reads the contract-baseline reference set |
| Project release | generic `release.md` → optional project `RELEASE.md`; if absent, stop with a planning route | same + task lifecycle | generic workflow → modified template/project contract → root `RELEASE.md` for this repository; CRUE release notes + update guide feed a later release | same generic/project split; accepted TS/RF/REVIEW and changelog/migration obligations feed the later project release |
| Trace landing | conventions exact-path/landing/value rules → producing workflow → review | same, task-bound | amended conventions → amended handoff/review and full copies | current conventions → current handoff/review; add only a narrow canonical clarification if Challenge finds a missing case |

C2 is not invalid merely because it has more hops: the extra guide could close an applicability gap. What is not yet seen is a unique job for `update-experience.md` that cannot live in the target update workflow and versioned migrations. The current update read contract already progressively selects only the applicable version range and major migration. A version-independent guide that also governs ownership, recovery and older-version prerequisites risks answering the same decisions as `update.md`.

Likewise, the revision-2 TS does not yet specify an authoritative read edge from a later `/tfw-release` to the CRUE task-local `RELEASE-NOTES.md`, nor a conflict rule if it differs from accepted RF/REVIEW or a versioned changelog. That file can be useful editorial input, but usefulness is not yet a unique authority/job.

### E3 — C0 closes repeatability only for detectable effects; it fails the preservation case

[RFC 9110 §9.2.2](https://www.rfc-editor.org/rfc/rfc9110.html#section-9.2.2) supports retry when the intended effect is idempotent or the client can determine that the original operation did not take effect. C0 can therefore safely re-run source comparison, copy parity checks and owner rendering if those effects are deterministic and current state is re-observed.

The counterexample is `UNTRACKED_LEGACY_README_AFTER_REPLACE`: a receiving `.tfw/README.md` contains project purpose, has no committed baseline, and is overwritten before a crash. Re-observation cannot reconstruct the exact prior bytes. Stopping every such update and requiring an owner-selected pre-existing location preserves safety but does not meet the approved routine transition outcome. A durable preservation entity is necessary unless the scope explicitly excludes unknown/untracked receivers.

A receipt alone is also insufficient. `STALE_SUCCESS_RECEIPT_AFTER_MANUAL_EDIT` shows why recovery must observe current receiver files and source identity before deciding that an earlier success still applies. This rejects a receipt-as-current-state design and supports C3's evidence-only receipt semantics.

### E4 — Task reuse is a real alternative, but it overloads lifecycle and retention

C1 answers the “no new carrier type” challenge more seriously than C0: an ordinary task already has a baseline, immutable commits, status, journal, evidence, RF and review. It could preserve bytes and explain an interrupted attempt.

Its named failure is `RECEIVER_UPDATE_WITHOUT_ACTIVE_TASK`: the canonical update command is invoked in a configured receiver where no TFW delivery task exists. Requiring task inception/approval/review before a routine framework update introduces a new authority and lifecycle precondition not present in the current route. Reusing an arbitrary existing task instead produces `UNRELATED_TASK_OWNS_PROJECT_MIGRATION`, where cleanup/archive or scope interpretation makes project-wide preservation depend on a task whose outcome is different.

C1 remains viable only if the owner deliberately redefines every update as a full task. No current source or external evidence demonstrates that this additional lifecycle improves correctness. It is therefore not the default minimal configuration, though Challenge should retain it as the strongest counterproposal to a new receipt type.

### E5 — One namespace may colocate two records, but their identities must remain separate

[W3C PROV-DM](https://www.w3.org/TR/prov-dm/) distinguishes entities, activities and agents, and treats provenance as information about how entities are produced and who bears responsibility. This is useful conceptual counterevidence to a single overloaded “update state” object:

- the legacy README attachment is an immutable **entity** whose exact bytes and hash matter;
- the update attempt is an **activity** with observations, effects and verification;
- owner, updater and source producer are **agents/authorities**, not fields that merge their mandates.

PROV-DM does not prescribe Markdown, `.tfw/update_receipts/`, a filename, a number of files, or a TFW permission model. It justifies maintaining the distinctions, not building C4.
...
The live contradiction is not optional. The project release template allows project-specific outputs and `None` as a version approach, while the generic workflow unconditionally imposes TFW `.tfw/VERSION`, changelog, SemVer and Git-tag mechanics. Root `RELEASE.md` already has the concrete mechanics for this repository.

The two Codex release skill files are byte-identical thin routers, so there is no duplicated release algorithm to consolidate. They still encode two entry assumptions: the description says “versioned release,” and the router requires the repository to contain `RELEASE.md`. If the repaired generic route must handle a non-versioned project or diagnose a missing optional contract, those two small files must change together. Their modification is a synchronization consequence with a demonstrated source sentence, not an independent design layer.

The minimum common release job is:

1. identify the requested output/audience and project contract;
2. distinguish prepare, integrate and externally publish effects;
3. verify project-declared readiness and actual state;
4. request authority only for missing consequential decisions/external effects;
5. report what is complete, not published, blocked or absent;
6. when no project contract exists, stop with a concrete `/tfw-plan` route rather than invent mechanics.

`NO_RELEASE_CONTRACT_DOCUMENT_PROJECT` and `NON_GIT_APPLICATION_RELEASE` eliminate both the current generic algorithm and a router that rejects before the canonical workflow can diagnose the case. `TFW_SELF_RELEASE` eliminates an empty generic workflow: this repository still needs root `RELEASE.md` to bind version, migration, payload, checks, tag and authorized publication.

### E8 — The plural Antigravity move is a source-consistency repair, not provider expansion

The manifest and adapter documentation already declare plural `.agents` targets; the tracked self-install tree remains singular `.agent`. Google currently documents workspace rules in `.agents/rules` while maintaining backward support for `.agent/rules` ([Google Antigravity Rules](https://antigravity.google/docs/rules-workflows)). Google also documents IDE workflows as Markdown slash-command sequences, while announcing migration toward skills by 1 November 2026 ([Google Antigravity Workflows](https://antigravity.google/docs/ide/workflows/)).
...
This is a Challenge input, not a final TS selector. “Keep” means retain the proposed action in the compact candidate; “merge/remove” means the job must be discharged by the named existing carrier or the failure case reopens it.

| Proposed entity or exact group | Provisional action | One job / authority | Displaced or reused carrier | Named failure if this disposition is wrong |
|---|---|---|---|---|
| `.tfw/README.md` | Keep MODIFY | Declare framework ownership while retaining approved methodology meaning | Replaces current preservation-by-default treatment in init/update | `CURRENT_VALUES_WITHHELD_AS_PROJECT_CUSTOMIZATION` |
| `.tfw/conventions.md` | Keep narrow MODIFY | Cross-workflow ownership/receipt lifecycle and only a demonstrated trace ruling | Reuse current North Star, landing, VALUE/TRACE and Candidate sections | `TWO_WORKFLOWS_INTERPRET_RECEIPT_OR_TRACE_DIFFERENTLY` |
| `.tfw/glossary.md` | Keep narrow MODIFY | Define receipt/PV terms once, without algorithm | Displaces duplicate definitions elsewhere | `RECEIPT_MEANS_CURRENT_STATE_TO_ONE_READER` |
...
### E10 — H5, H6 and H7 advance with different confidence
...
### E11 — External evidence and transfer limits at Extract

| Primary source | Extract use | Active limit/counterevidence |
|---|---|---|
| [RFC 9110 §9.2.2](https://www.rfc-editor.org/rfc/rfc9110.html#section-9.2.2) | Tests re-observation and idempotent/detectable repeat behavior | Does not select a receipt, preserve overwritten bytes, or make arbitrary writes retry-safe |
| [W3C PROV-DM](https://www.w3.org/TR/prov-dm/) | Keeps preserved entity, update activity and responsible agent analytically distinct | Does not prescribe a TFW file/schema, Git convention, authorization or repository-wide engine |
