...
1. **F-004 - native semantic effects remain unverified.** The two successful Claude reports are preserved and the current carrier reconciles read-only source/receiver/session/config/adapter/purpose/receipt facts. A root replay also reports preserved build/managed-file structures, but no build checks were run or passed, and purpose selection is not inferred from a keyword. Content-level semantic effects remain unverified. Owner comprehension is absent and explicitly disclosed, which restricts conclusions but is not a new field gate. AC-6/AC-8/AC-10 cannot be promoted to terminal success from this package; AC-9 retains its explicit limitation. This is not authorization for another campaign.
