# Local verification map — TFW_20260906-190312_CRUE / replacement Candidate

> Scope: maintainer/source and synthetic assurance only; no native subject or field slot.
> Prior Candidate remains reachable at `146876e279313a6a1a1680b0c7fffa699e30d28e`.

Each row names an input, action, observation and independent counterexample or explicit limit. A
source walkthrough is not reported as an executable behavioral case.

| Gate/case | Actual input and action | Observation | Independent counterexample or limit |
|---|---|---|---|
...
| AC-7 | Pre-repair command `python -m pytest tools/tests/ docs/scripts/ -q`: exit 0, `536 passed, 1 skipped in 516.81s`; replacement must rerun. | Maintainer route was green before repair. | Green maintainer tests are not receiver proof. |
...
| AC-12 self-hosting | Root `RELEASE.md` §6/§7. | Self-hosting procedure owns TFW payload metadata/migrations. | No release/tag/publication. |
| AC-13 partial/complete | Root §1/§6 selected-effect prerequisites. | Partial Executor work cannot be tagged; complete selected evidence can proceed. | Source-only walkthrough. |
| AC-13 root/phase evidence | Root §1 phase-only lineage. | Missing selected proof blocks selected effect, not all open tasks. | Source-only walkthrough. |
| AC-13 harmless/operational trace | Root §6.1 and generic §4 safety clauses. | Harmless trace may accompany checked work; operational trace stays gated. | No cross-unit landing. |
| AC-13 metadata order | `_local_release_order_from_source`; reorder mutant fails. | Prepare → verify final bytes/checks → commit → separate external authorization. | No release effect. |
| AC-13 target/tag mismatch | Root §6.1/§6.5 checked-commit/tag clauses. | Tag identifies only exact checked commit. | No tag. |
| AC-13 post-integration change | Root §6.4 post-integration check clause. | Operational changes require final-composition verification. | No integration. |
...
Native updater trajectories, provider/source transport isolation, real receiver variants, interruption
recovery, field questions/final messages and owner comprehension are not established. The field manifest
remains pre-freeze with zero consumed slots; these limits must remain BLOCKED/DEFERRED in EV.
