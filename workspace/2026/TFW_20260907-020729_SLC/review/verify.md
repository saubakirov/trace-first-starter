...
## C2 correction and final-output verification — 2026-09-10

Same Reviewer/direct LEAD and unchanged approved TS; exact C2 authority `d1ecc6994ea20fccddaaff57b7eb593ee490930f`, actual ONB acceptance `a24aa5b3c03ba33547ea623b2ee06fd1045b9a1a`, replacement Candidate `433d9db62905e16786113af9bfe126a89c21b9a5`, RF return `52159e4afdf961bc5f4077ef3cc011d98c05e0b1`, independent dispatch `b4a93a513035e05229a3822ab5581645cd0a3747`. Prepared merged input **`dd34cacfe88f095a28e67129fdde964e5760da66`** retains all ancestors; its product/test bytes equal Candidate. Read actual appended ONB/RF/EV, C2 ruling, return/accounting/checked-input/output records and complete three-file C2 diff. Original Map/PV and unaffected checks were not restarted.

| Check | Independently established result |
|---|---|
| Exact accounting | Executed the PowerShell block read from the TS at approval, after comparing it with the unchanged Executor script, into a fresh separate directory. **24 VALUE/+486−182=668**, 23 MODIFY/one ADD, zero rename or binary case; both raw files byte-equal Executor. Full 160-path inventory is 24 VALUE/six ASSURANCE/130 TRACE; same selector/556 denominator and 48/1112 bounds. [Accounting and raw hashes](../evidence/review-c2-final/accounting-result.json). |
| Source/lineage | Complete resolver and test diff inspected: existing/generated spans become opaque only during current substitutions, then restore; balanced/escaped/angle/title cases and marker collision are covered. Whole-ID lookup, precedence, phase/TD/D/backtick routes, collision handling and reference union retain their prior implementation. All 30 product/test Git blobs equal Candidate in the return/merge; C2 changed only gen_docs.py and its two allowed test homes. Candidate is the sole product commit after actual ONB and precedes EV/RF. |
| Acting producer/staging | Exact native Executor tool-call records show ONB acceptance at 07:40:52 UTC, pre-Candidate status at 07:46:17, captured file/cached checks at 07:49:44, explicit 19-path Candidate staging/cached-set assertion and `commit --only` at 07:50:04, and exact ten-path RF-return validation/commit at 07:57:42. Git paths/ancestry and subjects confirm the same producer and authority. Local rollout locator is the one recorded above; no reasoning was needed or reproduced. |
| Executor evidence | Independently checked **14** raw-index entries, including **eight** text-normalized working copies, **226** archived HTML pages, **35** route cases/**251** actual destinations and fragments. Fresh receipts are resolver85, collection590 and integration16/one MkDocs exit0. All **121** check-input hashes match; exactly the three authorized sources differ from original08-full. All **24** release constituents equal original Candidate. [Audit](../evidence/review-c2-final/executor-evidence-verification.json). |
| Reuse boundary | Reader/init/update/resume/native/state/adapters/PTTC source and relevant oracles are unchanged. Original08-full581/1 and Coordinator581/1 remain their own epochs; neither is a full C2 run. Fresh resolver/integration checks establish the changed function, while the next row establishes post-append final output. No native repetition or new full suite. |
| Actual final pipeline | One direct production `python -m mkdocs build --config-file docs/mkdocs.yml` on dd34cacf: **exit0, 158.12s**, all **2,276** tracked input hashes/HEAD unchanged. No fixture substitution or generated-output edit. [Manifest](../evidence/review-c2-final/input-manifest.json), [receipt](../evidence/review-c2-final/mkdocs.receipt.json), original stdout/stderr and raw transport are retained. |
| Actual final output | Independent standard-library parser opened **43** current artifact/control pages, examined **369** local links and verified **45** current HL routes; all retained35 C2 route cases still resolve. New C2 RF/EV text and the actual return/review-dispatch events render. Four KNOWLEDGE rows retain **14** valid links and code-form guide paths. **51** opened HTML files and their targets are archived/hash-verified. [Output](../evidence/review-c2-final/opened-output.json), [archive](../evidence/review-c2-final/opened-html.tar.xz). |
| Remaining limitations | **96** current raw-site omissions all resolve real repository files and retain ruled O8 limits. The sole other selected negative is unchanged approved TS's3.2.0-guide href at HTML3729; the same3366 negative and absent predecessor/current manifest entry were checked. Source/guide bytes and release route survive. This is separately proposed O9, not a new all-links-clean claim or silent O8 extension. |

The own HTML archive SHA256 is **`893738009050e6728f13f5030e50f2156e0b4dfbd2fecc7cdfe70e28d9d9b5c7`**. [Integrity](../evidence/review-c2-final/integrity.json) verifies ten original evidence files and the raw capture SHA256 **`ddd35668a6addf1ef46b7d86cdc45078a03f97abff9b90ed2ab0b1c7e36d62c5`**; raw bytes remain available independently of Git CRLF normalization. This archive is transport for actual retained evidence, not another product/runtime mechanism.

After verification, independently reread frozen master HL §1/§3/§7 and current designated NS1–NS3. The restored route serves readable/citable history and inspectable continuity; it introduces no unrelated subsystem or purpose/authority conflict. RF's empty fact/insight/diagram additions were challenged against the actual change; no new human input or missing explanatory edge was found. The exact local correction and observable destinations establish O7 completion. O7 payment and O9 ruling remain Coordinator acts; no remaining cited Candidate defect was found.

One audit initially addressed the nested build receipt as `returncode`; inspecting the actual schema showed `exit_code`, which was then validated with the original bytes. An umbrella North Star heading lookup was absent; the actual declared NS1/NS2/NS3 headings were read directly before judgment. These read-only diagnostics did not change the source/output or create successful test epochs. The actual build had no failure or interruption. The verdict/carrier recording itself is later acceptance TRACE under D86, not falsely part of dd34cacf's rendered bytes and not a trigger for recursive output rebuilding.
