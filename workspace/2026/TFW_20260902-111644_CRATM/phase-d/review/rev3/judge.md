...
|---|---|---|---|
| 1 | DoD / all TS AC | ❌ | Rev2 AC-1–AC-5 hold. Rev3 AC-6 fails at the committed-review boundary because its continuation guard rejects the required rev3 stage traces; AC-7 checkbox 4 also fails because `duplicate(BASE)` neither includes nor unambiguously covers the rendered `LEAD_BASE`; Verify V13–V15. |
...
| 8 | Evidence is sufficient | ❌ | Green navigation evidence has no collision inputs, branch, case or mutant, and the continuation boundary is red after the prescribed rev3 review traces are committed. It cannot establish AC-7's retained collision/no-key behavior or AC-6's legal-continuation boundary. |
| 9 | Backward compatibility | ❌ | Ordinary `BASE` collision behavior remains, but the newly rendered root form does not inherit it. Concurrent selected-root titles can collide without the existing stable-key suffix/fail-soft safeguard AC-7 promised to preserve. Separately, landing the workflow-required rev3 stage traces makes the Phase-D continuation assurance fail. |
...
| Phase-D continuation assurance admits the rev3 REVIEW sibling but rejects the workflow-required `review/rev3/{map,verify,judge}.md` traces | The repository's own focused Phase-D guard turns red as soon as the prescribed review record is committed, so the legal post-Candidate continuation boundary is not complete | TS rev3 AC-6 checkboxes 2, 4 and 6 | **Rung 1 — correction inside the approved TS** |
...
4. In `docs/scripts/test_integration.py`, make the Phase-D continuation allowlist admit exactly the
   workflow-required rev3 REVIEW sibling and `review/rev3/{map,verify,judge}.md` TRACE files while
   continuing to reject arbitrary product, assurance and unrelated trace paths. Prove the focused
   boundary at the committed post-review tip.
5. Preserve the approved 21 VALUE selector, two ASSURANCE paths, principal/unit/mandate separation,
...
6. Because item 1 is later VALUE and items 2–4 are later ASSURANCE, create a replacement tested Candidate,
...
These are two proposals, not an acceptance ruling or implementation order. They return directly to the
...
- [x] Verify V13–V15 are cited in the DoD assessment?
