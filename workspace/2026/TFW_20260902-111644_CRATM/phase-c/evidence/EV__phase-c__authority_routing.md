...
+## 8. Return Round 1 Evidence

> Ruling: 2d6e0f95696d12ed044305d4cf99b78d292338da
> Replacement Candidate: 989240a4714925ff9eaeb198d9f046c32f451d63
> Baseline: fb08c120a91aca4c9ceaea859d46dd49c032afd0
> Immutable denominator: 12 VALUE files / 320 touched LOC

### 8.1 Return evidence verdict

| Row | AC | Result | Resolving artifact |
|---|---|---|---|
| R1-E1 | AC-1 | VERIFIED | Parsed Plan consumer below routes ordinary CL before prefix validation |
| R1-E2 | AC-2 | VERIFIED | Fresh complete authority validator: 38/38 decisions and 10/10 mutants |
| R1-E3 | AC-3 | VERIFIED | Human exceptions remain explicit; zero universal owner readers |
| R1-E4 | AC-4 | VERIFIED | Current census, Plan projection/mutant, and six copy hashes below |
| R1-E5 | AC-5 | VERIFIED | Fixed caps, route/corpus JSON, 637-pass suite and real build |
| R1-E6 | AC-6 | VERIFIED | Contemporaneous commit capture, 12/186 replay, lineage and no-later-VALUE |
| R1-E-accounting | AC-6 | VERIFIED | Exact approval/ruling/Baseline/Candidate, selector, NUL output, 94 + 92 = 186, 12 files, binary N/A, unchanged authority/timing |

Return Round 1 verdict: **7/7 VERIFIED, 0 DEFERRED, 0 BLOCKED, 0 N/A**.

### 8.2 Contemporaneous replacement-Candidate capture

The following is the untouched successful shell output, normalized only from console CRLF to Markdown
LF. It was produced after the 637-pass run and before EV/RF/state append. A prior wrapper had a
PowerShell parse error before executing any statement; it staged and committed nothing and is not
misrepresented as a successful capture.

```text
=== PRE-COMMIT FULL STATUS ===
COMMAND: git status --short --branch
## HEAD (no branch)
M  .agent/workflows/tfw-plan.md
M  .claude/commands/tfw-plan.md
M  .tfw/conventions.md
M  .tfw/workflows/plan.md
M  docs/scripts/test_integration.py
M  docs/scripts/test_runtime_context.py
=== PRE-COMMIT CACHED NAMES ===
...
M	docs/scripts/test_integration.py
M	docs/scripts/test_runtime_context.py
=== PRE-COMMIT UNSTAGED NAMES ===
COMMAND: git diff --name-status
=== EXACT FULL-PATH PATHSPEC ===
.tfw/conventions.md
.tfw/workflows/plan.md
.agent/workflows/tfw-plan.md
.claude/commands/tfw-plan.md
...
git commit -m "[codex/TFW_20260902-111644_CRATM/phase-c/executor] implement return round 1" --only -- .tfw/conventions.md .tfw/workflows/plan.md .agent/workflows/tfw-plan.md .claude/commands/tfw-plan.md docs/scripts/test_runtime_context.py docs/scripts/test_integration.py
[detached HEAD 989240a] [codex/TFW_20260902-111644_CRATM/phase-c/executor] implement return round 1
 6 files changed, 184 insertions(+), 14 deletions(-)
=== GIT-DERIVED REPLACEMENT CANDIDATE ===
989240a4714925ff9eaeb198d9f046c32f451d63
=== POST-COMMIT FULL STATUS ===
## HEAD (no branch)
```

### 8.3 Complete return validator and exact byte boundaries

Extraction command:

```python
from pathlib import Path
text=Path("workspace/2026/TFW_20260902-111644_CRATM/phase-c/evidence/EV__phase-c__authority_routing.md").read_text(encoding="utf-8")
marker="```python authority-validator-return-1\n"
program=text.split(marker,1)[1].split("\n```",1)[0]
exec(compile(program,"<EV return-1 authority-validator>","exec"),{})
```

```python authority-validator-return-1
from __future__ import annotations
import json
from dataclasses import dataclass,field
from pathlib import Path
...
expected={k:{"decision":v[0],"ruler":v[1],"refusal_reason":v[2]} for k,v in AUTHORITY_EXPECTED.items()}
mutants=authority_mutant_results(tree)
plan_consumer=plan_authority_consumer_payload(tree)
assert set(payloads)==set(expected)==set(actual)
assert all((v["decision"],v["ruler"],v["refusal_reason"])==AUTHORITY_EXPECTED[k] for k,v in actual.items())
assert all(v["projection_changed"] and v["independent_expected_rejects"] for v in mutants)
assert plan_consumer["cases"]["ordinary_cl"]["ruler"]=="owner-human"
assert plan_consumer["mutant"]["projection_changed"] and plan_consumer["mutant"]["independent_expected_rejects"]
print(json.dumps({"candidate":"989240a4714925ff9eaeb198d9f046c32f451d63",
"contract":authority_contract(tree),"payloads":payloads,"expected":expected,
"actual":actual,"mutants":mutants,"plan_consumer":plan_consumer},indent=2,sort_keys=True))
print(f"SUMMARY fixtures={len(payloads)} authority_parity={len(actual)} authority_mutants={len(mutants)} plan_cases={len(plan_consumer['cases'])} plan_mutant_rejected={int(plan_consumer['mutant']['projection_changed'] and plan_consumer['mutant']['independent_expected_rejects'])}")
```

Exact stdout representation is UTF-8 bytes emitted by executing the program, including its one
terminal LF. Length: 108192. SHA-256:
71f852a74b19efb803033e9d7bb265b3029221f93158be92297db4bd339e2381.

The JSON fence below contains stdout with exactly that terminal LF removed because the Markdown
closing fence owns the separator newline. Fenced-content length: 108191.
Fenced-content SHA-256: 44e2878f7e5aa95a3423cec2f07e31a83871ca25825a01cd0d23bc00c06fc74f. Proven relation:
fenced_bytes + b"\n" == stdout_bytes. The earlier round-zero byte-for-byte/fenced digest claim is
superseded and withdrawn from live evidence; its rejected historical text is preserved.
...
```json
{
  ...
  "candidate": "989240a4714925ff9eaeb198d9f046c32f451d63",
  "contract": {
    "accountability_forbidden": true,
    "child_only": true,
    "coordinator_only": true,
    "nearest": true,
    "old_to_new_guarantee": true,
    "owner_explicit": true,
    "owner_fallback": true,
    "owner_from_status": true,
    "preserve_proposer": true,
  ...
  "plan_consumer": {
    ...
    "mutant": {
      ...
      "produced": {
        ...
        "case": "ordinary_cl",
        "decision": "REFUSE",
        "refusal_reason": "ordinary-cl-requires-delegation",
        "required_facts": [
          "human status owner",
          "signer"
        ],
...
The parser executes the actual Plan 6d bullet order. Ordinary CL requires only human status owner and
signer and routes to owner-human. Delegated input uses every prefix fact. The actual Plan mutation
changes ordinary CL from ROUTE to REFUSE and is independently rejected.

Census command:

```text
git grep -n -i -E owner|ruler|verdict 989240a4714925ff9eaeb198d9f046c32f451d63 -- .tfw/conventions.md .tfw/workflows/plan.md .tfw/workflows/review.md .tfw/workflows/handoff.md .tfw/templates/HL.md .tfw/templates/RES.md
```

Raw current-source output:

```text
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:59:| HL section | State after owner approval |
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:65:1. **The contract state is artifact state.** The HL header carries a `Contract` field with two values: `📝 DRAFT — not yet approved` and `🔒 FROZEN — approved by {owner} YYYY-MM-DD`. Task status tracks the pipeline; the `Contract` field tracks the artifact. They are not interchangeable.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:66:2. **Free sections stay free.** Research and the coordinator update §2, §7.2, §8, §9, §10 and §11 directly, with no proposal and no verdict. Risk registers, hypothesis statuses and dependency statuses are required to move.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:67:3. **A frozen section may not be edited.** The only channel is §12 Amendment Log: propose, resolve and record the rule-8 verdict, then apply. This includes the coordinator that authored the HL.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:72:8. **A verdict is a distinct, resolved act.** Chat/workflow input is evidence, never a verdict. The governing task/phase `status.md.owner` must be a declared human and supplies the root/fallback ruler; a separate governing record authorizes the root Coordinator. Old: “only the owner rules.” New ordinary delegation: nearest eligible non-proposer, else governing owner. Ordinary CL without delegated claim routes to that owner. For claimed delegation, a task/phase-local `dispatch` edge is its `writer` → destination plus governing scope/role references. Only a Coordinator on one unambiguous human-rooted prefix may add a new child. Before work refuse an Executor source, unknown/repeated/competing node or parent, ancestor/task-Coordinator target, missing root authorization, or non-human/unresolved termination. Preserve the originating proposer through transcription and sessions. For ordinary `EXTEND`/`SUPERSEDE`, walk upward from the proposer: skip `false` grants and the same handle; choose the nearest remaining immutable `true` principal, otherwise the owner. Before signing, validate chain, proposer, grant, reservation and signer; gaps or contradictions stay `PROPOSED` and block. Profile role, `accountable_to`, binding, title, provider, `writer`, or `on_behalf_of` never supplies root, path, proposer or grant. Owner-reserved claims and an agent's own grant/handle change route to the owner.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:73:9. **An owner-initiated frozen change is an amendment too** — one §12 row records the owner as `Proposer` and their real explicit decision. An agent's `on_behalf_of`, human binding or `accountable_to` cannot create this direct-human exception. The log's value is the record, not the gate.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:74:10. **A restrictive change applies on filing.** Narrowing — adding a DoF item, tightening scope, dropping a deliverable — is logged with `Type` = `RESTRICT` and verdict `✅ APPLIED — no owner verdict required`. Restrictive-free is prohibited: the classifier benefits from the label, so the log costs nothing and removes the incentive.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:75:11. **`Type` states relation to the baseline, never disposition.** `EXTEND` adds and the original stays in force; `SUPERSEDE` replaces; `RESTRICT` narrows. Disposition belongs in `Verdict`.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:144:Formal coordinator report after reviewing RF: checklist, verdict, and a disposition on every debt item it captured.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:192:> The EV file captures environment metadata, per-AC verification results, and a verdict summary.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:199:| EV file | `evidence/EV__{...}.md` | Observational / Verification | Environment header, per-AC evidence table, verdict, attachments |
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:200:| RF | §5 Evidence (pointer) | Summary / Reference | One-line pointer to EV file + verdict summary |
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:247:the owner approves both before a directory is created; the HL header carries them side by side
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:250:code a person cannot read back — and never created without the owner's approval. A title is
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:254:owner-approved abbreviation. It never recomputes the timestamp, adds a suffix or silently
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:408:| **REVIEW** | **sibling** | Exactly one verdict is live, and the highest ordinal is it |
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:470:Every AI-authored commit MUST use `[agent/task/scope/role] summary`: set `agent` to the lowercase AI product name from explicit context, `task` to the canonical TFW task ID (`project` only when none exists), `scope` to the established lowercase work-slice slug or a lowercase hyphenated form of its explicit label, and `role` to the lowercase canonical TFW workflow owner from §15/Role Lock; keep `summary` short and imperative, commit locally, and push only after explicit user approval.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:489:not define a merge strategy; one mutation owner controls each worktree.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:638:| ❌ REJECTED | Task closed unsuccessfully and permanently. Distinct from ❌ BLOCKED, which is waiting and resumes when the dependency clears. Terminal: no status follows it, and the task folder and its board row are never deleted. This is a task status — not the review verdict ❌ REJECT, and not the HL §12 amendment verdict ❌ REJECTED; neither of those is terminal |
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:643:**`UNDECLARED`: migration never normalizes; an accountable owner may resolve.**
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:648:| The task's owner setting the correct value and recording a `transition` event with `from: UNDECLARED` | **Yes.** The accountable decision and its trace are explicit |
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:666:Review verdicts:
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:669:  once, then follows **The 🔄 REVISE route** below. The verdict alone never moves lifecycle
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:679:verdict artifact; recording a Coordinator ruling there is acceptance control, not a new
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:686:| Rung 3 | a frozen HL claim | Coordinator, then `HL Contract` rule-8 ruler | HL §12 proposal plus `amendment_escalated` event and resolved ruler's terminal verdict | none until that verdict leaves an executable bound | unchanged; Executor is not dispatchable | Reviewer → Coordinator → resolved ruler; **STOP until terminal verdict** |
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:689:A REVISE item names the failed TS acceptance criterion or frozen HL claim, its owner, and an
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:693:valid terminal verdict leaves an executable bound. The Executor appends ONB, RF, and EV
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:695:with the remainder disposed, or transition to `BLOCKED` and return to the task owner because no
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:696:basis can be stated. An `unassigned` owner is a hard stop. A fresh role holder resolves lineage
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:763:| Owner escalation multiplier | 2 | `owner_escalation_multiplier` | Delegated boundary against each immutable planned measure |
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:766:verdict, and pre-work ref. Compare forecasts/Candidate with the immutable owner plan; no ruling ratchets it.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:767:Owner rules before work at/above multiplier or from planned zero. Below it, Coordinator may add only a
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:768:necessary constituent while Goal, Value, outputs, AC, DoF, phase/ownership, architecture/target,
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:809:| [review.md](workflows/review.md) | Reviewer | Read RF → checklist → verdict → debt disposed → traces |
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:941:| Category | Files | Init | Update | Owner |
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:1057:- Anyone edits a frozen HL section without a §12 row carrying the valid rule-8 verdict — the silent contract edit the amendment channel exists to replace
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:1059:- Coordinator applies an amendment before its valid rule-8 verdict — the proposal and change become one act, so the ruling follows what it governs
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:1061:- Any role treats a remark inside a research thread, a review or a chat as an amendment verdict — a comment is input, a verdict is a distinct recorded act
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/conventions.md:1144:When a Reviewer reaches a verdict, the correct action is to **name the next act** — a decision with
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:5:> **Title**: {Title — the full title the owner approved}
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:9:> **Frozen**: §1 · §3 · §4 · §5 · §6 · §7 — locked on owner approval
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:14:> **Contract field** — one line, two states. Until the owner approves:
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:16:> `🔒 FROZEN — approved by {owner} YYYY-MM-DD` and commits the file before research starts.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:17:> A frozen section may not be edited afterwards: propose in §12, wait for the verdict.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:47:> §3.1 is a gate, not an illustration: the owner's checkpoint **before** the spend of tokens and
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:233:> A frozen section may not be edited before its row carries a verdict. Rows are never deleted,
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:239:> **A remark is input, never a verdict.** Preserve the originating proposer through Coordinator
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:244:> **Owner-initiated** uses rule 9 only for the real human owner's explicit act on that row. Owner-reserved
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:245:> claims and an agent's own grant/change-of-handle route to the owner.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:251:>   change applies **on filing** and is logged with the verdict `✅ APPLIED — no owner verdict
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:254:> **`Verdict` values:** `PROPOSED` (awaiting a ruling) · `✅ APPROVED — {ruler}, YYYY-MM-DD` ·
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:255:> `❌ REJECTED — {ruler}, YYYY-MM-DD` · `✅ APPLIED — no owner verdict required` (`RESTRICT` only) ·
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:258:> credit the owner with a decision they never made).
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:266:| # | Date | § | Type | Proposer | Proposed change | Evidence | Cost | Alternatives considered | Verdict |
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/HL.md:268:| A1 | YYYY-MM-DD | §{n} | `EXTEND` / `SUPERSEDE` / `RESTRICT` | {owner / coordinator / research iterN / executor} | {what changes} | {where the finding comes from} | {what it costs to accept} | {what else was weighed and why it lost} | `PROPOSED` |
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/RES.md:36:> routes by `conventions.md` → `HL Contract` rule 8. Frozen claims wait for a valid terminal verdict.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/RES.md:46:### Amendment Proposals — frozen sections, resolved-ruler verdict required
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/RES.md:48:> Same columns as HL §12 except `Date`, originating `Proposer`, and `Verdict`, which the Coordinator
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/templates/RES.md:49:> adds; verdict starts `PROPOSED`. Reassign local `A1`, `A2` … into append-only §12. Evidence, cost,
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/handoff.md:68:3. **Rung 3:** accept only after `HL Contract` rule 8 resolves a valid terminal verdict leaving an
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/handoff.md:79:revision, or a rung-3 round without the resolved ruler's valid terminal verdict is not executable. Record the missing authority
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/handoff.md:119:prompts and require their recorded terminal disposition, not a quality veto. The owner-approved VALUE
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/handoff.md:121:forecast at or above `owner_escalation_multiplier`, or growth from an applicable planned zero. The
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/handoff.md:148:    explain every non-VERIFIED row, summarize the verdict counts, and index any attachments.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/handoff.md:156:    - RF §5 says `See [EV file](...) for evidence details.` plus verdict summary.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/handoff.md:163:    including §5 as an EV pointer plus verdict summary and §7–§9 with explicit `No …` when empty.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:72:   owner approves both in this exchange, before any task directory is created. Never invent a
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:93:   if dir exists: STOP; ask the owner to approve a different abbreviation
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:136:1. Set the HL header `Contract` field to `🔒 FROZEN — approved by {owner} YYYY-MM-DD`
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:179:   - frozen claim → transcribe into HL §12 with verdict `PROPOSED`; the section itself stays untouched
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:193:### 6d. Amendment verdicts — whenever one arrives, in research, ONB, review or execution
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:195:- **No delegation claimed:** validate the human status owner and signer, then route directly to that
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:196:  owner; root, chain and grant facts are inapplicable.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:197:- **Delegation claimed:** resolve rule 8 before applying; verify owner, root authorization, child-only
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:199:- **✅ Approved by the resolved ruler** → record the signed §12 verdict, apply, then commit the new `freeze` baseline
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:200:- **❌ Rejected** → the row keeps its verdict and stays; the original contract holds; resume work
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:201:- **`RESTRICT`** → rule 10 applies on filing; **owner-initiated** → rule 9 requires the real human owner's explicit act
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:220:6b. Get owner approval of the TS and immutable VALUE denominator. Suggest `/tfw-handoff`; repeat per phase.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:228:   PROPOSAL. Retain the cited criterion/claim, owner, and observable completion; no basis fails.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:231:   and waits for a valid terminal verdict before Executor dispatch.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/plan.md:233:   name amendment/ruler and "STOP until terminal verdict." Never execute the round.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:9:> **Output:** REVIEW file with verdict + a disposition on every debt item it captured
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:14:> Never modify implementation; fundamental defects go in REVIEW with verdict ❌ REJECT.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:27:| 5 | Decide | stage files; `.tfw/conventions.md` headings `Task control files`, `Session identity`, `Artifact file naming`, `Task Statuses`, `The 🔄 REVISE route`, `Safety and Execution Honesty`, `Trace Discipline`, and `Role Lock Protocol`; `.tfw/templates/REVIEW.md` | identity, verdict, disposition, routing, trace | stage/shared rule/template |
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:104:immutable owner-approved denominator. The Reviewer may not ratchet the plan, construct a different selector,
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:105:supply missing Coordinator/Owner authority after work, or invent a competing total. Missing, mutable,
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:138:> **Mindset:** Decision-maker. Synthesize stages into a binding verdict with cited proof.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:141:Write `REVIEW__*.md` from its template: synthesize §1–§3; §4 gives the evidenced APPROVE/REVISE/REJECT verdict.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:144:pass; both route to the **owner**, never the executor (`judge.md` row 2a). Rung 3 follows `The 🔄
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:149:acceptance criterion, or a frozen HL claim; the rest is disposed of in §5. Cite nothing and the verdict is
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:150:✅ APPROVE, the remainder disposed. Neither cite nor approve and the work returns to the task's `owner`
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:154:membership, arithmetic, timing, or authority disagreement, and route it through the existing verdict rules.
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:176:## Step 6: Record verdict, then route proposals
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:182:After verdict:
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:183:1. **Set the task's own state only when the verdict authorizes it** — APPROVE enters `KNW`; REJECT
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:184:   follows its selected owner route; REVISE alone does not move lifecycle. Every actual transition
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:187:2. **Check §5** — every item carries one of the three dispositions. An undisposed item blocks `DONE`, not the verdict
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:194:After ✅ APPROVE verdict:
989240a4714925ff9eaeb198d9f046c32f451d63:.tfw/workflows/review.md:202:**Hard stop:** after the verdict and its authorized trace/KNW routing are recorded, stop. Never
```

Allowed-class summary:

| Occurrence family | Classification |
|---|---|
| Canonical rule 8 old-to-new disclosure and ordinary/delegated routes | replaced amendment route |
| Conventions section 14 valid rule-8 verdict/application lines | replaced amendment route |
| Plan 6d explicit no-delegation and delegation branches | replaced amendment route |
| Rule 9, rule 10, owner-reserved/self-grant, Purpose/defect/REJECT | preserved human exception |
| Identifier, freeze, approval denominator, budget multipliers | unrelated budget/approval rule |
| CHANGELOG, KNOWLEDGE, prior task traces | historical and excluded from live enforcement |

Universal owner-reader detector output: [].
The two section 14 lines now require a valid rule-8 verdict/application and contain no owner-signing
reader. The historical old guarantee is explicitly paired with the new guarantee in canonical rule 8.

### 8.5 Accounting, lineage, copies, caps, tests
...
```json
{
  ...
  "numstat_exit": 0,
  "numstat_raw_nul_escaped": "3\t3\t.agent/workflows/tfw-handoff.md\\014\t14\t.agent/workflows/tfw-plan.md\\04\t2\t.agent/workflows/tfw-review.md\\03\t3\t.claude/commands/tfw-handoff.md\\014\t14\t.claude/commands/tfw-plan.md\\04\t2\t.claude/commands/tfw-review.md\\012\t9\t.tfw/conventions.md\\09\t6\t.tfw/templates/HL.md\\010\t20\t.tfw/templates/RES.md\\03\t3\t.tfw/workflows/handoff.md\\014\t14\t.tfw/workflows/plan.md\\04\t2\t.tfw/workflows/review.md\\0",
  "numstat_records": [
    {
      "additions": 3,
      "deletions": 3,
  ...
  "phase_attribution": "all 12 resolve to approved Phase C selector; otherwise INVALID",
  "replacement_candidate": "989240a4714925ff9eaeb198d9f046c32f451d63",
  "ruling_ref": "2d6e0f95696d12ed044305d4cf99b78d292338da",
  "trigger_disposition": "12/186 below 50/5000; one unchanged Phase C"
}
```
...
```json
{
  "ancestry": {
    "approval_to_ruling": {
      "ancestor": "1f1173d968e9b74a5e06e3e2070ae604c2844ca5",
      "descendant": "2d6e0f95696d12ed044305d4cf99b78d292338da",
      "exit": 0,
      "passes": true
    },
    "old_to_replacement": {
      "ancestor": "b2a963670e2587cffa6a61d8851f37065f03cda9",
      "descendant": "989240a4714925ff9eaeb198d9f046c32f451d63",
      "exit": 0,
      "passes": true
    },
    "ruling_to_candidate": {
      "ancestor": "2d6e0f95696d12ed044305d4cf99b78d292338da",
      "descendant": "989240a4714925ff9eaeb198d9f046c32f451d63",
      "exit": 0,
      "passes": true
    }
  ...
  "candidate_to_capture_tip_value_diff": "",
  "parent": "f904e3af4ccb87ebc466ec2d896c25fa66b694be",
  "producer": "Executor / same task",
  "subject": "[codex/TFW_20260902-111644_CRATM/phase-c/executor] implement return round 1"
}
```
...
[
  {
    "canonical": ".tfw/workflows/plan.md",
    "canonical_sha256": "3d7439e86191d4ad6a9b0417ba07aa7aca3f560a5e83c6a35f98d7400d987352",
    "copy": ".agent/workflows/tfw-plan.md",
    "copy_sha256": "3d7439e86191d4ad6a9b0417ba07aa7aca3f560a5e83c6a35f98d7400d987352",
    "equal": true
  },
  {
    "canonical": ".tfw/workflows/plan.md",
    "canonical_sha256": "3d7439e86191d4ad6a9b0417ba07aa7aca3f560a5e83c6a35f98d7400d987352",
    "copy": ".claude/commands/tfw-plan.md",
    "copy_sha256": "3d7439e86191d4ad6a9b0417ba07aa7aca3f560a5e83c6a35f98d7400d987352",
    "equal": true
  },
  {
    "canonical": ".tfw/workflows/review.md",
    "canonical_sha256": "0082008bd307635a7d6632d078ddc7753d1881c7782a69a51b489c8b2fae58ce",
    "copy": ".agent/workflows/tfw-review.md",
    "copy_sha256": "0082008bd307635a7d6632d078ddc7753d1881c7782a69a51b489c8b2fae58ce",
    "equal": true
  },
  {
    "canonical": ".tfw/workflows/review.md",
    "canonical_sha256": "0082008bd307635a7d6632d078ddc7753d1881c7782a69a51b489c8b2fae58ce",
    "copy": ".claude/commands/tfw-review.md",
    "copy_sha256": "0082008bd307635a7d6632d078ddc7753d1881c7782a69a51b489c8b2fae58ce",
    "equal": true
  },
  {
    "canonical": ".tfw/workflows/handoff.md",
    "canonical_sha256": "5c1faf2f0b61935a1273071ea5d7e8f49cacedb98580a2f67c071fdab2cd676d",
    "copy": ".agent/workflows/tfw-handoff.md",
    "copy_sha256": "5c1faf2f0b61935a1273071ea5d7e8f49cacedb98580a2f67c071fdab2cd676d",
    "equal": true
  },
  {
    "canonical": ".tfw/workflows/handoff.md",
    "canonical_sha256": "5c1faf2f0b61935a1273071ea5d7e8f49cacedb98580a2f67c071fdab2cd676d",
    "copy": ".claude/commands/tfw-handoff.md",
    "copy_sha256": "5c1faf2f0b61935a1273071ea5d7e8f49cacedb98580a2f67c071fdab2cd676d",
    "equal": true
  }
]
...
Unchanged cap spans and route/corpus output:

```json
{
  "cap_spans": {
    "PHASE_C_PRIMARY_ENTRY_WORDS": {
      "candidate_sha256": "5e5bc216b0520e0843b56214f5f0973abdc9a3632a21213fa14b064bb5c6e81d",
      "equal": true,
      "literal": "PHASE_C_PRIMARY_ENTRY_WORDS = {\n    \"/tfw-plan\": 24_730,\n    \"/tfw-research:focused\": 6_103,\n    \"/tfw-research:deep\": 6_168,\n    \"/tfw-handoff\": 6_366,\n    \"/tfw-review\": 25_182,\n}",
      "ruling_sha256": "5e5bc216b0520e0843b56214f5f0973abdc9a3632a21213fa14b064bb5c6e81d"
    },
    "SESSION_ROUTE_CEILINGS": {
      "candidate_sha256": "87a076a1e9fb2849ffae9f27734208ee933cd865649e053795d3cd3a8c34ba4d",
      "equal": true,
      "literal": "SESSION_ROUTE_CEILINGS = {\n    \"/tfw-plan\": 24_725,\n    \"/tfw-research:focused\": 6_102,\n    \"/tfw-research:deep\": 6_167,\n    \"/tfw-handoff\": 6_366,\n    \"/tfw-handoff:revise\": 6_366,\n    \"/tfw-review\": 24_954,\n    \"/tfw-resume\": 3_264,\n    \"/tfw-docs\": 15_278,\n    \"/tfw-init\": 4_529,\n}",
      "ruling_sha256": "87a076a1e9fb2849ffae9f27734208ee933cd865649e053795d3cd3a8c34ba4d"
    }
  },
  "route_report": {
    "active_corpus": {
...
```text
python -m pytest docs/scripts/test_runtime_context.py -q -k "cratm_phase_c or test_phase_c_every_changed_path_and_active_corpus_clear_thirty_percent or test_rtpsn_phase_b_context_routes_corpus_and_local_caps_do_not_grow"
8 passed, 171 deselected in 69.41s; exit 0

Direct final integration census/copy/mutant replay
2 checks passed; exit 0

python -m pytest .tfw/scripts/ docs/scripts/ --collect-only -qq
638 collected; exit 0

python -m pytest .tfw/scripts/ docs/scripts/ -q
637 passed, 1 skipped in 334.64s (0:05:34); exit 0

python .tfw/scripts/gen_index.py --check project
project consistent with framework 2.1.0; exit 0

git diff --check
no output; exit 0
```

The configured suite includes the real MkDocs build fixture. No cap, manifest, plural target,
runtime, schema, Phase D/E, external state, push, or master mutation occurred. At evidence capture,
Candidate equals HEAD and Candidate-to-tip VALUE diff is empty. Later return writes are TRACE only;
any later VALUE invalidates this replacement Candidate.
