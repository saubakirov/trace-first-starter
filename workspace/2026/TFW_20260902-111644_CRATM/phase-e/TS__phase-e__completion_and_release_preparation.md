# TS — TFW_20260902-111644_CRATM / Phase E: Completion and release preparation
...
Finish the frozen Phase E product after its reviewed integration checkpoint: consolidate the dated F11
correction, remove the three remaining live TFW-54 promises and synchronize their six installed copies,
add five glossary routers, discharge the material citation/debt census, and deliver a mechanically
replayable 3.0.0 release package. Then obtain an independent final review, close the production task
honestly, and apply the already-verified release package in a separate post-DONE release commit without
tag, push, publication, or disturbance of unrelated saved-checkout work.
...
- Replace only the three current canonical future-TFW-54 sentences in Handoff, Research, and Review:
  optional `writer` records the acting principal only when **Who Is Acting** resolves one; otherwise the
  field is omitted. Never infer `robert`, never create a profile per session, and never conflate
  principal attribution with unit identity, role, proposal origin, or authority.
- Byte-synchronize the corresponding three `.agent/` and three `.claude/` full-copy receivers.
- Add glossary routers for `Principal`, `Initiation Chain`, `Worktree Protocol`, `Landing Commit`, and
  `AT (Agent Team)`; each points to one normative owner and duplicates no procedure.
- Fix the Phase B B9 heading link. Verify the already-correct master NS2 ordinals. Record that the
  `.agents/` plural in the Antigravity contract is intentional and that tracked `.agent/` is a distinct
  installed-copy surface; do not edit the manifest or normalize either spelling. Dispose the retired
  Phase-D `gen_index`/immutable-event-length observations as non-material after RTBO.
- Create one VALUE-classified task-local release-package carrier containing the exact final 3.0.0
  migration, changelog/updating texts, version values, destination mapping, digests, pre/post checks,
  and replay commands. Verify the package against a disposable tree while canonical release files stay
  unchanged during Candidate II.
- Extend only the two named assurance files for current behavior, package replay, parity, regression,
  negative cases, snapshot/current separation, and accounting. Preserve every historical assertion.
- Produce EV, RF, formal independent REVIEW, approved docs/knowledge postconditions, and honest Phase E
  closure. After DONE, invoke `/tfw-release`, obtain Main's exact technical confirmation of the six-path
  write set/baseline, apply the verified package, verify it, and commit it separately.

### Out of Scope

- Any new runtime, required Python/PyYAML, tracked workspace index, execution mode, profile, artifact
  class, coordination unit, thread, task, fork, subagent, or provider admission.
- A complete Claude-only TFW role chain or a route into another provider's existing long-lived task.
- Editing historical references that truthfully describe TFW-54 as future at their original epoch.
- Manifest/config spelling normalization, new Antigravity paths, or removal of `.agent/` receivers.
- Canonical release-file writes before Phase E is independently approved and DONE; tag, push,
  publication, deployment, or a claim that 3.0.0 exists before the separate release commit is verified.
- Executor or Reviewer access to the saved checkout; modification, staging, cleanup, or adoption of its
  unrelated tracked template hunk, untracked feedback files, or SLC task.
...
### 4.1 K1 — exact `/tfw-knowledge` correction

The literal K1 VALUE selector is:

```powershell
$k1ValuePaths = @(
  '.tfw/knowledge_state.yaml',
  'knowledge/constraint.md',
  'workspace/2026/TFW_20260902-111644_CRATM/HL-TFW_20260902-111644_CRATM.md',
  'workspace/2026/TFW_20260902-111644_CRATM/phase-d/HL__phase-d__team_mode_and_role_assignment.md',
  'workspace/2026/TFW_20260902-111644_CRATM/phase-d/RF__phase-d__team_mode_and_role_assignment.md',
  'workspace/2026/TFW_20260902-111644_CRATM/phase-d/REVIEW__phase-d__team_mode_and_role_assignment.md',
  'workspace/2026/TFW_20260902-111644_CRATM/phase-d/REVIEW__phase-d__team_mode_and_role_assignment__rev2.md',
  'workspace/2026/TFW_20260902-111644_CRATM/phase-d/REVIEW__phase-d__team_mode_and_role_assignment__rev3.md',
  'workspace/2026/TFW_20260902-111644_CRATM/phase-d/REVIEW__phase-d__team_mode_and_role_assignment__rev4.md',
  'workspace/2026/TFW_20260902-111644_CRATM/phase-e/HL__phase-e__sweep_correction_and_release.md'
)
```
...
The exact F11 replacement is:

> **Provider session topology is measured per installed surface; independently addressable peers are not Codex-only.** On 2026-09-03, Claude Code exposed eight owner-opened peers in two listings twenty minutes apart; one three-day-old peer in another project answered a delegation-shaped message. The same installed surface created and resumed a headless `claude -p` session, and fresh Codex→Claude and Claude→Codex crossings both ran without a TFW-side mechanism. These probes did not validate a complete Claude-only TFW role chain. These observations do not establish an in-scope route from either provider to the other provider’s existing long-lived role task, so the first CRATM release uses provider-homogeneous long-lived chains and treats cross-provider fresh runs only as bounded helpers. Dispatch checks the actual surface, tools, version, and OS; provider capability evidence supplies neither identity nor authority.

- Verified: `✅ re-measured 2026-09-03 and bounded 2026-09-05`
- Sources: `RES CRATM iter2 D1–D3; RES CRATM iter3 D4/D5/D7/H10/H12; frozen Master HL §2 “A dated capability fact needs a narrower replacement” and A6`
- Updated: `2026-09-07`

| K1 accounting fact | Approved value |
|---|---|
| Subject | Exact F11 replacement plus complete selected-section marker/state reconciliation |
...
The literal Candidate-II VALUE selector is:

```powershell
$candidateIIValuePaths = @(
...
$candidateIIAssurancePaths = @(
  'docs/scripts/test_integration.py',
  'docs/scripts/test_runtime_context.py'
)
```

| Candidate-II accounting fact | Approved value |
|---|---|
| Subject | Remaining E product and complete verified 3.0.0 release package; exact literal 12-path selector |
| Baseline | `b0bfcd22125d8a34366d7eb885a2fb54234bdc7d`; K1 must be an ancestor but is a separately accounted selector |
...
hand and are not counted as nine new paths in the final unique-path union.

### 4.3 K2 — post-review docs and knowledge reconciliation

The literal K2 VALUE selector is:

```powershell
$k2ValuePaths = @(
  'KNOWLEDGE.md',
  '.tfw/knowledge_state.yaml',
  'workspace/2026/TFW_20260902-111644_CRATM/phase-e/RF__phase-e__sweep_correction_and_release.md',
  'workspace/2026/TFW_20260902-111644_CRATM/phase-e/REVIEW__phase-e__sweep_correction_and_release.md'
)
```

K2 may start only from the exact commit containing the independent APPROVE verdict, after Main checks
and records that immutable SHA. `/tfw-docs` may update only `KNOWLEDGE.md` §§1–3 from the accepted
RF/REVIEW; `/tfw-knowledge` may mark the final RF/REVIEW selected sections and update state last. Any
additional knowledge topic, changed fact count, or changed selector requires a new exact ruling.

| K2 accounting fact | Approved value |
|---|---|
| Subject | Accepted final docs/knowledge postconditions after independent review |
| Baseline | Exact final-REVIEW commit, recorded and technically confirmed before K2 writes |
...
The literal release VALUE selector is:
...
The package must prescribe: create the 3.0.0 migration; add an explicit supersession note to 2.2.0;
...
|---|---|
| Subject | `/tfw-release` application of the verified Candidate-II package |
| Baseline | Exact post-DONE/K2 commit, recorded and technically confirmed by Main before canonical writes |
...
The prior integration selector is the exact 25 paths in
`TS__phase-e__integration_gate.md`. K1 adds 10 distinct paths. Candidate II has nine integration
overlaps and therefore adds three unique paths. K2 overlaps `KNOWLEDGE.md` with integration and
`.tfw/knowledge_state.yaml` with K1, therefore adds two. Release adds six. The exact final union is
`25 + 10 + (12 - 9) + (4 - 2) + 6 = 46` unique VALUE paths.

| Whole-result accounting fact | Approved value |
|---|---|
...
Gate: exact path/numstat diff; F11 literal comparison; marker census; state statistics/digest; before/after
`KNOWLEDGE.md` blob; `python tools/tfw_doctor.py knowledge-pending`.

Evidence: K1 commit SHA and Coordinator transcript recorded in the Phase E EV/RF.

### AC-2: The three live TFW-54 promises become correct optional-principal rules
...
Evidence: real Candidate-II tree and parity/census output.

### AC-3: Glossary and debt corrections are minimal and complete [depends: AC-2]

- [ ] The five named glossary terms exist as routers to one normative owner each and contain no copied
  procedure or second source of truth.
- [ ] Phase B B9 links to `#11-strategic-insights-planning-free`; every link/anchor check passes.
...
- [ ] `phase-e-3.0.0-release-package.md` contains final, non-placeholder text for both migration files,
  changelog/updating material, all three version values, six destinations, operation order, expected
  digests, pre/post checks, rollback boundary, and exact verification commands.
- [ ] Package routes users from earlier supported tags, quotes only the retired instruction needed to
  identify its supersession, retains historical entries, and makes no unsupported quantitative claim.
- [ ] Applying the package to a disposable tree from the named pre-release baseline yields exactly the
  six-path expected release diff and passes package tests; canonical release destinations remain
  byte-identical to `b0bfcd2…` at Candidate II.

Gate: package parser/replay in a disposable worktree or temporary Git tree; exact six-path/digest diff;
protected canonical release blobs at Candidate II.

Evidence: package content plus replay output in Phase E evidence.

### AC-5: Candidate II is fully assured and honestly accounted [depends: AC-1–AC-4]

- [ ] Candidate II is the first fully tested Executor VALUE commit after K1 and exact approval. Its
  12-path VALUE and 2-path ASSURANCE selectors match exactly; no accepted output is hidden as TRACE.
- [ ] Full configured collect/test, targeted Phase E, strict configured MkDocs build, `git diff --check`,
  canonical/copy parity, stale-site census, release-package replay, negative mutants, historical snapshot
  guards, and current-tree assertions all pass. A non-configured warning mode may be reported but cannot
  substitute for the configured gate.
- [ ] Integration snapshots are compared to their immutable Candidate-I objects; current behavior is
...
- [ ] The same distinct Reviewer task independently reconstructs baseline, selector, candidate, tests,
  parity, package replay, protected history/current separation, and saved-work boundary from its own
  checkout; it writes the formal review stages and final REVIEW.
...
- [ ] `KNOWLEDGE.md` §§1–3 receive only accepted RF/REVIEW outcomes; final RF/REVIEW markers and state
  reconcile exactly. Any new topic/fact/count or extra path stops for a new ruling.
- [ ] Final postconditions are checked; Phase E and master task transition through required states to
  DONE with append-only journals. DONE means implementation and the verified package are ready for the
  separate local release; it does not claim version files, tag, push, or publication already changed.

Gate: K2 path/accounting/digest checks, `/tfw-docs` and `/tfw-knowledge` postconditions, lifecycle
transition validation, absence of canonical release changes.
...
- [ ] Coordinator invokes `/tfw-release`, checks the exact post-DONE baseline, bump to 3.0.0,
  ...
  exact technical confirmation before canonical release writes.
- [ ] Migration 3.0.0 is created first, then the 2.2.0 supersession note, changelog entry/updating text,
  and three version values are applied exactly from the verified package and committed separately.
...
- For workflow edits, modify canon first and copy the complete canonical bytes to both accepted
  receivers. Assertions should cover no binding, matching binding, unrelated binding, AT/no-AT, root/
  child, forwarded context, and cross-provider-capability non-authority.
- Treat package verification as source generation: a disposable tree may receive the six writes, but
  the Candidate-II worktree's canonical release paths must stay protected. Store the exact proposed
  bytes/digests and deterministic replay order in the package carrier.
...
- ❌ Candidate II leaves a live current TFW-54 promise, changes historical true-at-the-time text,
  infers `robert`, conflates principal/unit/role/authority, or breaks any canonical/copy triple.
- ❌ The manifest or canonical `.agents/` contract is "corrected" to `.agent/`, unrelated config drift is
  normalized, or retired RTBO/Phase-D history is rewritten.
- ❌ The release package contains placeholders, incomplete updating routes, unverifiable digests, or
  unsupported claims; canonical release files change before DONE.
- ❌ An accepted package, knowledge, docs, or release output is classified as TRACE or excluded from its
  subject; checkpoint LOC totals are summed as the final actual; whole-tree snapshot comparison rejects
...
|---|---|---|
| Three canonical workflows and six copies | CRATM integration, RTBO, A–D | Preserve current combined semantics; change only live writer promise |
| `knowledge/constraint.md`, `KNOWLEDGE.md`, state/source markers | Master/D and prior knowledge passes | K1 literal F11 first; K2 accepted docs/markers only |
| Phase B HL | Phase B reviewed history | One broken B9 anchor; no contract rewrite |
| Release/migration/config paths | RTBO and earlier releases | Post-DONE only; full updating path and saved foreign hunk preserved |

> References: frozen master A6/A8, §2, Phase E deliverables 1–6, DoD 15–17, DoF 1–12;
> `knowledge/constraint.md` F2/F11/F12; `knowledge/process.md` F30/F39–F41; integration TS/EV;
> Main release-order and exact-F11 technical rulings, 2026-09-07.

---

*TS — TFW_20260902-111644_CRATM / Phase E: Completion and release preparation | 2026-09-07*
