...
| `~/.tfw/bindings.yaml` on the owner's machine | ⬜ absent — Phase B will be its first real use. Measured free: it is read by no shipped script, so a principal key costs nothing |
...
| `gen_index.py` | 🚫 **not a dependency.** Owner ruling 2026-09-03: the generator was a mistake, is being retired elsewhere, and is to be ignored here — no phase reads it, edits it, or gates on it |
...
Closed by the two iterations of 2026-09-03: what the tools do at the mechanics level · whether peers
are a transport · what a delegate must be told · whether the two role dimensions are right · whether
a revision round should prefer a fresh participant. What remains, and what the research opened:
