...
**Both crossings work, with no TFW-side mechanism, and they are not symmetric.** Codex → a fresh
Claude is one command. Claude → a fresh Codex needed three config overrides on this machine plus
`</dev/null`, then returned cleanly in 16 s and 43,466 tokens — an install artifact of a stale CLI
against a newer config, so it says something narrow about this machine today rather than about Codex.
**Neither vendor can address the other's existing session at all.** A dispatch is therefore
send-and-wait with follow-up by id, never a live conversation — and the token floor agrees: one word
cost 43,466 tokens, which prices whole-workflow delegation near 40 K per phase, per-stage near 170 K,
and a coordinator↔delegate chat at 0.4–1.3 M.
...
- **Whether the worktree protocol says anything about session identity per run.** Phase A isolates the
  ...
  nothing more.
- **Whether a current Codex build removes the three config overrides** the forward crossing needed.
  If it does, the crossing is one command and the cost model changes; if not, the cross-vendor rows of
  a Role Assignment carry a setup cost nobody has written down. Cheap to re-measure after the owner's
  next upgrade.
