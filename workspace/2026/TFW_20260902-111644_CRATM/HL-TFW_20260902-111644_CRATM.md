...
> **Abbreviation**: CRATM
> **Status**: 🧩 PHASES — Phase A planning; master contract frozen
...
Research iteration 2 replaced the coordinator's first reading with measurement. **Eight peers listed,
twice, twenty minutes apart**, six of them three days old, three sessions of one tool open in this
project at once. One three-day-old peer **in a different project** answered a delegation-shaped
message in under a minute with substantive state. So peers are real and durable.

But a peer **cannot be created from inside a session** — a human opens it. So a Role Assignment
written before the work starts cannot name one. The creatable, durable delegate is a **headless
resumable session**: `claude -p` returns a `session_id`, persists as a transcript, and answered from
its own history on `--resume` across a process boundary — 5.7 s, ~33.7 K cached tokens. That is the
capability Phase D's promise actually rests on, and it is not the one the coordinator assumed. The
honest degradation below it is a subagent, which is a stage helper and not a delegate.

**Both crossings work, with no TFW-side mechanism.** Codex → a fresh Claude is one command. Claude →
a fresh Codex was one command too, once the owner upgraded the CLI mid-research: `codex-cli 0.120.0`
→ `0.152.1` turned three config overrides into none — exit 0, 10 s, **5,030 tokens** against the
43,466 measured hours earlier on the older build, and 368 bytes of output against 307,553. **Neither
vendor can address the other's existing session at all.** A dispatch is therefore send-and-wait with
follow-up by id, never a live conversation.
...
And the general finding outlives both numbers: **a cross-vendor route's cost is a property of
installed versions**, measured in both directions of the same claim on one machine in one day. A
Role Assignment naming two vendors inherits a version-skew failure mode → §9 R15.

F11's correction is Phase E's to write, and this is the text it needs.

### Intervening baseline — RCFR landed; VBSA is accepted but not landed

The original Phase A draft was written before RCFR. At `master` `8034d72` on 2026-09-05, RCFR is
`DONE` and D73–D75 have replaced universal context loading with workflow-owned ordered reads and one
adapter manifest. Across Phase A's three canonical targets, that landing made 202 insertions and 314
deletions. It did **not** add a worktree, staging, or landing rule.

VBSA Phase A has separately passed review at `9221dbb`, but its 19 commits are not in `master` yet;
the task is `PHASES`, Phase A is `KNW`, and Phase B is `ONB` in its active worktree. Its accepted
candidate changes the same three canonical files and their four tracked adapter copies. Phase A
therefore plans against that accepted candidate, but execution waits until VBSA closes and lands.

| File | Phase A draft | RCFR in `master` | accepted VBSA candidate | Consequence |
|---|---:|---:|---:|---|
| `.tfw/conventions.md` | 11,269 words | 9,269 | 9,791 | Reference owner remains suitable; add each rule once |
| `.tfw/workflows/handoff.md` | 1,727 | 1,751 | 2,013 | The inherited file is further above F2; no duplicate rule body |
...
operations. A Codex coordinator can create and control user-visible tasks; the measured Claude setup
required the owner to provision peer sessions before control could be delegated. Treating those as
one topology would move the owner back into the routing path.
...
| VBSA — Value-Bearing Scope Accounting | 🟠 accepted Phase A is still unlanded; task `PHASES`, Phase A `KNW`, Phase B `ONB`. CRATM Phase A may be planned now but is not dispatched until VBSA is `DONE` and landed |
...
| R10. A delegated task returns silence, and the coordinator replaces it, splitting one role across several task ids — observed in field history | Medium | Medium | Wait once; inspect the referenced durable artifact and the workspace; steer or interrupt the **same** task; replace only on evidence of termination, and record the new id if it happens |
| R11. **A Role Assignment row promises a channel the vendor pair cannot open.** `Channel: visible thread` works only when coordinator and delegate share a vendor: there is no route from a Codex session to a named Claude peer, nor from a Claude session to a live Codex thread | High | Medium | Phase D states the constraint where the column is defined, and the achievable channels are named: same-vendor thread, headless session by id, or a human-opened peer addressed by name |
...
| R14. The headless session is measured on one word, not on a workflow | High | High | Before Phase D's TS: one real delegation of a small approved phase, measured end to end. Named as an open thread rather than assumed |
| R15. **Version skew between two vendors' installed CLIs is a live failure mode of the mode itself.** Measured twice on one machine in one day, in both directions of the same claim: an upgrade from `0.120.0` to `0.152.1` turned three required config overrides into none and cut a call from 43,466 tokens to 5,030 | High | Medium | A Role Assignment names participants, never versions, so nothing in the contract goes stale. Phase D states that a cross-vendor row's operational cost is environment-dependent and unversioned by design, and that a route measured once is not a route guaranteed |
...
| R17. **The two iterations reported different installed Codex versions on one machine** — `0.151.0-alpha.7.2` from iteration 1, `0.120.0` before the upgrade from iteration 2. Either two installs coexist or one measured a different surface | Medium | Medium | Not resolved by either iteration. Phase D's TS establishes which install a cross-vendor row means before it promises a channel |
...
| R19. CRATM and the unlanded VBSA branch edit the same canonical and adapter files | High until landing | High | Plan only against VBSA's reviewed candidate; do not dispatch CRATM Phase A until VBSA is `DONE`, landed, and the main tree is clean |
| R20. A request for a peer session is implemented as a nested subagent or an agent-definition file | High | High | The provider profile defines an observable boundary: the role holder has its own provider-visible session/task identity and can be listed and addressed directly by its coordinator; a child behind a relay fails the gate |
| R21. Claude-only autonomy begins before the owner has provisioned the peer sessions its coordinator cannot create | High | High | Provision every required peer before the autonomy boundary, prove `ListAgents`/`SendMessage` routing once, and stop rather than degrade a team role into a relay or subagent |
| R22. A live session roster is introduced to repair addressability, then becomes a second state authority and drifts | High | High | Iteration 3 must demonstrate routing from provider-native live discovery plus frozen role artifacts, or report that Claude-only cannot satisfy DoF 3 and 7; no `SESSIONS.md`-like carrier is added |
| R23. A mixed-provider team makes the owner relay messages because neither coordinator can address the other provider's existing sessions | High | High | Candidate boundary: first release contains homogeneous provider profiles only; mixed execution stays out until a later task proves direct routing without the owner |

## 10. RESEARCH Case 🟢 FREE

### Blind Spots

The two iterations of 2026-09-03 closed the mechanics they asked about. The Helpdesk field run of
2026-09-05 opened a different layer: whether the mode must have provider-homogeneous execution
profiles rather than one mixed topology. What remains:

- **The observable boundary between a peer session/task and a nested subagent.** Provider vocabulary
  is not cosmetic: in Helpdesk, `ListAgents`/`SendMessage` reached parent sessions while the intended
  role holders remained hidden behind relays. In Codex, user-visible tasks/threads and the in-task
  subagent tree are likewise different planes.
- **Whether Claude-only can carry TFW without inventing a live roster or returning the owner to the
  loop.** The owner can provision peers; the untested part is whether a Claude coordinator can then
  discover, address and govern them through a complete approved unit while every participant obeys
  its TFW Role Lock and artifact boundary.
- **Whether the successful Codex RCFR/VBSA practice is the first shippable profile rather than merely
  more anecdote.** Their task/worktree traces are materially stronger than iteration 1's read-only
  nested probe, but have not been synthesized against a peer-session acceptance test.
- **Whether mixed-provider execution belongs in this task at all.** Cross-provider fresh calls work;
  cross-provider existing-session conversation does not. The current candidate is to exclude mixing
  from the first release and research it later only if it has independent value.

- **Whether a headless session can carry a whole workflow.** It was measured on one word, not on
  `/tfw-handoff` for a real phase with its context loading and its artifacts. Phase D's promise rests
  on this, and nothing yet supports it beyond a round trip. → R14.
...
- **Whether the worktree protocol says anything about session identity per run.** Phase A isolates the
  ...
  nothing more.
- ~~Whether a current Codex build removes the three config overrides~~ — **closed the same day** by
  the owner's own upgrade: it does, all three, and the cost fell an order of magnitude. What the
  closure exposed is a standing blind spot instead: **which install a cross-vendor row means.** The
  two iterations reported different versions on one machine, and a route measured on one build is not
  a route on another.
...
Both iterations complete 2026-09-03. Statuses below are the researchers'; the coordinator applied
them without changing a verdict.
...
| H10 | A Codex-only team can use independently addressable user-visible tasks/threads for every long-lived TFW role, while subagents remain bounded stage helpers rather than role holders | 🟡 **field-supported, not yet synthesized** — the owner reports RCFR and VBSA followed the intended chain; their durable task/worktree traces exist, while iteration 1 tested only nested read-only children |
| H11 | A Claude-only team can satisfy the same provider-neutral contract if the owner provisions peer sessions before autonomy and the coordinator thereafter uses Claude-native discovery and messaging without relay subagents or a live roster | 🟠 **needs a real TFW trial** — Helpdesk proves peer provisioning and direct session work are possible, but also records hidden subagents, owner corrections, relay routing, and a stale roster |
| H12 | Mixed-provider execution cannot satisfy the walk-away promise with current session controls and should be outside the first release | 🟡 **strong negative evidence** — both crossings start fresh runs, but neither provider addresses the other's existing session; Helpdesk shows what happens when an unaddressable role is placed behind a relay |
...
H10–H12 now decide whether Phases C and D specify one false universal topology or two honest
provider-homogeneous profiles. Without the distinction, a Claude role can silently become a hidden
subagent behind a relay, a mixed team can return the owner to message routing, and a compensating live
roster can violate the frozen artifact and liveness boundaries. The earlier H1–H9 results remain
inputs; they do not answer this field-discovered question.
...
#### Candidate Iteration 3 — pending owner approval

**Claude measures a homogeneous Claude topology; Codex evidence is compared, not mixed into the
run** (H10–H12). Start from the Helpdesk field trace, especially its session roster and TFW artifacts,
then run one bounded, already-approved TFW unit through owner-provisioned peer Claude sessions. The
trial must show the provider-native list and message operations, prove that every long-lived role
holder is a directly addressable session rather than a nested subagent or relay, and audit the actual
ONB/RF/REVIEW/HL/TS boundaries it obeyed. It may not introduce a live roster into CRATM.

The same RES then evaluates RCFR and VBSA as existing Codex-only field traces against the identical
observable criteria: user-visible task identity, coordinator addressability, role separation,
worktree isolation, artifact legality, and owner interventions. Mixed-provider execution is only a
negative control: record the missing direct route and the owner-routing consequence; do not launch a
mixed team. The output is a recommendation for two provider-homogeneous profiles and a verdict on
whether mixed execution is deferred to another task. This is one sequential third iteration under
the normal semantics, assigned to `claude-code`; it starts only after the owner approves it.
...
| S24 | **The owner proposes no mixed-provider autonomous team for now.** **Implication:** research first decides the two homogeneous profiles; mixing is a later independent question, not an automatic combination of two profiles | stakeholder | User, 2026-09-05, proposed pending confirmation |
