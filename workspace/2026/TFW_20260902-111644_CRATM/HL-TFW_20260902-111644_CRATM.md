...
> **Refreshed**: 2026-09-07 by Main Coordinator (Codex), applying owner-selected Robert mandate A8
> **Title**: Contextual Roles and Agent Team Mode
> **Abbreviation**: CRATM
> **Status**: 🧩 PHASES — master contract frozen; each phase owns its live state
> **Contract**: 🔒 FROZEN — approved by saubakirov 2026-09-07 (A8; product A7 2026-09-06; original approval 2026-09-02)
...
### 4.1 Actual CRATM Role Assignment 🔒 FROZEN

This operational assignment is separate from §3.1's product illustration. Owner selection A8
continues the already approved autonomous Codex-only arrangement after research and the committed
A7 contract. It applies from 2026-09-07, not to the attribution of historical A–D work.
...
|---|---|---|---|---|---|---|
| `robert` | `saubakirov` | Remaining approved CRATM Phase E: sweep, correction, adapter parity, integrated local verification, version/changelog preparation and reviewed closure; preserve accepted A–D and separately approved RTBO and participant changes | Main root coordinates one new Phase E Coordinator, which creates one Executor and one distinct Reviewer; exactly those three E units, native Codex tasks in separate worktrees, no forks or subagents. Main decides routine technical planning and acceptance within the frozen contract; children retain their own workflow Role Locks | Exact E TS and immutable VALUE plan before execution. Human retains strategy, changed goals/outputs, reserved claims, root-origin amendments, grant changes, scope/budget escalation and replacement of an unavailable assigned holder. No provider mixing, new runtime, per-child profile, machine-wide binding, release tag, push or publication authorized here | Human owner in root Codex task `01a07050-9d35-7080-a5f6-afd14334e68d`; every child reports directly to its recorded parent through native task messages | `PHASES`, prospectively after A8 freeze; child execution only after its exact approved TS and bounded dispatch |

The selected root may use `team/robert.md`'s immutable `may_rule_amendments: true` eligibility only
for genuinely subordinate-origin, non-reserved proposals inside this mandate and the governing
contract. This is not permission to widen its own authority or excuse a scope/budget overrun.
Children share attribution, never that root-only ruling grant. Robert's mentality guides the
working style; it is not authentication, a new permission or a change to any workflow lock.
...
|---|---|---|---|---|---|---|---|
| `robert` | Coordinator — Main root | `01a07050-9d35-7080-a5f6-afd14334e68d` | Human owner `saubakirov` in this root task | This selected mandate; appoint and coordinate E's Coordinator, not execute or review its work | Direct owner chat in this task; native Codex task messages to actual children | `PHASES`, after A8 freeze | [root activation](journal/20260907-010722__dispatch__5cf0.md) |
| `robert` | Coordinator — Phase E | `01a07856-6a45-7211-93fd-1b79d7bfed62` | Root `01a07050-9d35-7080-a5f6-afd14334e68d` | Plan E; first resolve its current Knowledge Gate through a source-backed candidate report. No product integration, source markers or knowledge-state writes before the applicable exact approval | Native Codex task messages directly to Main root | `PHASES` for read-only prerequisite and planning; execution remains TS-gated | [E planning dispatch](journal/20260907-011832__dispatch__695c.md) |
...
| 39 | PV 0 — [NS2 principle 5](../../../.tfw/README.md#ns2) | Human authority, bounded delegation | A8 records the real owner's selected principal, current root, bounded reach and retained decisions before child work |
| 40 | PV 1 — [Methodology values](../../../.tfw/README.md#methodology-values) | Naming Creates Behavior; Structural Enforcement | Only the recorded root renders `LEAD · robert · CRATM`; child role titles and actual dispatch records stay distinct |
| 41 | PV 2 — [philosophy F37](../../../knowledge/philosophy.md); PV 3 — [D83](../../../KNOWLEDGE.md); PV 4 — [HL Contract rules 8–9](../../../.tfw/conventions.md#hl-contract) | Mandate ceiling; principal/unit separation; real owner-initiated act | A8 is prospective owner selection, not self-authorization from a profile, title, binding or historical A7. Existing approvals and proposal origins are not rewritten |
...
| A8 | 2026-09-07 | §4.1 actual CRATM Role Assignment | `EXTEND` | Owner — saubakirov | Select existing principal `robert` for the current Main Coordinator root and continue the already approved Codex-only, worktree-isolated Phase E delegation. Record the actual root, bounded mandate and later append-only child assignments; do not change product scope or retrospectively rename A–D actors | Direct owner act in root task `01a07050-9d35-7080-a5f6-afd14334e68d`: *«применяй к себе роберта … продолжаем … к фазе Е»*, together with the standing explicit instruction for one new Codex phase Coordinator, its one Executor and one independent Reviewer, direct task messages and technical decisions by Main. The owner separately approved the participant files preserved by commit `3153c5d12528bc5bf859333f5d17097fc04b4d46` | One prospective mandate and root dispatch; no new profile, runtime, global binding, product claim or external publication. Subsequent E work still needs its exact approved TS and immutable VALUE plan | Title-only rename — rejected because navigation is not authority. Machine-wide Robert binding — not requested. New personal profile per child — rejected by A7 and the owner. Reopen A–D — unnecessary: retain their reviewed history | ✅ APPROVED — saubakirov, 2026-09-07 |
...
> **A8 records a new owner act, not a grant inferred from A7.** The owner now selects Robert for
> this actual root task. A7's historical statement that it created no profile or session remains
> true. This selection applies prospectively; the profile's mentality guides style only, and all
> inherited workflow locks, acceptance gates and human reservations remain in force.
