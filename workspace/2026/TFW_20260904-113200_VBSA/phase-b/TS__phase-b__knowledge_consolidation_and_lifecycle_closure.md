...
**Cause:** the first tested Candidate `1b336e1257a87e6552090f549cfa3381614ec6d2` reproduced the approved 1-file/5-LOC result, but the tracked assurance suite reported `560 passed, 1 skipped, 2 failed`; both failures were immutable `/tfw-plan` context-ceiling assertions (`24,935 > 24,730`), and the `+207` contribution was the newly required §1 Config/D76 content. **Cost:** replace the adjacent existing one-line D75 row with a shorter one-line form while preserving its decision, metrics, and authoritative links; the replacement Candidate remains exactly one VALUE path and is expected to measure `5` additions + `2` deletions = `7` touched text LOC. Actual work is authorized only strictly below the owner boundary: one VALUE file and at most `9` touched text LOC. **Assurance effect:** the Executor must rerun the full tracked suite and all AC/accounting/link/protected-boundary checks; both context-ceiling tests must be green before the replacement Candidate. **Split alternative:** a second carrier or phase would duplicate authority and cannot reduce the context graph; changing the ceiling/test/canonical reader is out of scope. **Saint-Exupéry judgment:** same-file subtraction is the smallest change that preserves the accepted result and its executable guard. **Authority:** prospective Coordinator ruling relaying Main Coordinator task `01a06b11-b421-74f3-a8da-6008f88ab38e`, before any D75 edit, recorded in `journal/20260905-121000__dispatch__eeb8.md`. **Verdict:** `APPROVE` only for the bounded D75 replacement described here. If D75's decision, metrics, or links cannot be preserved, if the suite remains red, or if actual accounting reaches `10` LOC, a second VALUE path, another section, or a protected-boundary change, stop and route to the owner. The first Candidate is superseded and must not be reviewed; after this ruling the Executor creates one replacement Candidate and performs the complete evidence/RF replay. This R1 supersedes only the TS's exact four-row/`4 + 1 = 5` assertions for the replacement Candidate; the required Config, D76, §2, §3 effects, immutable owner-approved denominator, and every other boundary remain unchanged.

#### R2 — larger existing-row subtraction after proof (2026-09-05)

**Cause:** before editing D75, the Executor proved that the immutable ceiling required a reduction of `205` counted tokens while D75 contained only `61`; even an empty structural D75 row would leave the graph `149` tokens above the ceiling. **Cost:** leave D75 byte-identical and replace exactly one other existing §1 decision row in the same `KNOWLEDGE.md`; D70 is preferred because its current `429`-word size permits removal of roughly `220` words while retaining about `200` words and full provenance. The replacement Candidate remains expected at `5` additions + `2` deletions = `7` touched text LOC and is authorized only through `9` touched text LOC. **Assurance effect:** before the edit, the Executor must record source-derived before/after word counts and a retention checklist covering the bold decision, every currently active non-negotiable rule, the rationale, and every existing link; after the edit, it must rerun the exact `/tfw-plan` ceiling assertions, the full tracked suite, and all AC/accounting/link/protected-boundary checks. **Split alternative:** a second carrier cannot shrink the read graph; changing a ceiling, test, canonical reader, or more than one existing decision row is outside this ruling. **Saint-Exupéry judgment:** removal of repetition and procedural detail already recoverable through the linked RF, REVIEW, and field reports is the smallest viable subtraction; semantics, history, metrics, and provenance cannot be removed. **Authority:** prospective Coordinator ruling relaying Main Coordinator task `01a06b11-b421-74f3-a8da-6008f88ab38e`, before any additional-row edit, recorded in `journal/20260905-121325__dispatch__16a2.md`. **Verdict:** `APPROVE` for exactly one proven semantics-preserving existing-row replacement, preferably D70; R2 supersedes R1's D75 target but preserves R1's replacement-Candidate/full-replay route and every unchanged boundary. If one-row subtraction cannot make the full suite green without semantic or provenance loss, stop and route to the owner.
