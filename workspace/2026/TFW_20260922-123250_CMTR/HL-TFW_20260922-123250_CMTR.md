...
5. **Iteration 3 дал дисциплину решения, а не оптимизатор.** Кандидат: точный результат/материальный отказ → необходимая capability/context/tools → отдельно достаточный поддерживаемый effort → lower resource только среди обоснованно достаточных пар → свежий native binding и фактический dispatch → коррекция следующего запуска по принятому результату и rework. При неизвестной стоимости optimum не заявляется; при недостаточной capability или assurance допустим результат «не запускать». H1–H4 остаются open/partial до независимой iteration 4 и последующих разрешённых пилотов.
...
| # | Hypothesis | Status |
|---|---|---|
| H1 | Решение «нижняя граница качества → минимальная достаточная стоимость» устойчивее ролевой трехуровневой классификации на реальных TFW-запусках. | open — есть аналитические контрпримеры к tiers, но нет equivalent-work outcome comparison |
| H2 | Раздельный выбор способности модели и reasoning effort предотвращает ошибки, которые скрывает единый профиль. | open — структурная и native-contract поддержка без причинного сравнения |
| H3 | Применение выбора непосредственно в dispatch надежнее замороженного поля HL/TS и не требует нового постоянного артефакта. | partial — Codex parameter passing и delivery доказаны; prospective rule и effective settings ещё нет |
| H4 | Для каждой заявленной платформы можно назвать авторитетный источник доступности и точный автономный либо owner-assisted способ запуска. | partial — поверхности разведены, но account selectors и foreign-provider chains не пилотированы |
...
1. **Independent attack:** проверить, различает ли rationale iteration 3 более дешёвую правдоподобную пару или превращается в default-max.
2. **Native readback:** отделить requested pair от resolved/effective setting и честно оставить недоступное ненаблюдаемым.
3. **Provider boundary:** атаковать claims Antigravity/Claude на предмет selector, visible unit, exact return и account availability без запуска неразрешённых пилотов.
4. **Prospective value:** определить, какое accepted outcome/rework может подтвердить или опровергнуть выбор в Phase C; не считать сам RES доказательством собственной policy.
