...
Перед любым exists/type/read доступом к registry или lock операция заново проверяет полный pinned component chain и private owner/ACL существующего `tfw-assisted` namespace. Если namespace ещё не создан, сначала перепроверяется pinned existing parent; после создания namespace получает private owner/ACL или Unix mode и заново включается в полный chain. Все registry reads выполняются только после этой проверки и под live OS lock; namespace substitution, permissive ACL или непрочитанный owner означают ноль registry reads и ноль registry/lock/temp writes. Запись registry выполняется через same-directory temporary, flush/fsync, replace, permission recheck и post-read под тем же lock. Возраст lock-файла не разрешает удалять или обходить чужую блокировку. Новый shared profile создаётся только после совпавшего profile manifest и свободного target, затем перечитывается.
