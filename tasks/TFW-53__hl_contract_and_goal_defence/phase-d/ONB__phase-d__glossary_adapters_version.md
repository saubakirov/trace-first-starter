...
> **Status**: 🟠 ONB — **answered 2026-08-14**, cleared to execute. Q1 → (a) out of mandate · Q2 → (a) rename the field label · Q3 → (a) re-route TD-158. All six recommendations approved except R6, overruled as a recorded coordinator scope extension. Six TS measurements corrected from this file
...
| # | Question | Answer |
|---|----------|--------|
| 1 | **AC-2's `committed baseline` gate cannot be satisfied without a forbidden rewrite.** Measured: the phrase never occurs as a standalone term. Both hits are the substring inside **`uncommitted baseline`** — `conventions.md`:75 (*"An uncommitted baseline makes 'frozen' permanently unverifiable"*) and `plan.md`:60 (*"an uncommitted baseline cannot be diffed"*). These name the **absence** of the thing, not a synonym for it, so substitution yields nonsense and any fix is a sentence rewrite — which DoF-1 and AC-7 forbid. Which? **(a) Out of mandate (recommended)** — `uncommitted baseline` is not a third name; AC-2's gate is read with the negation excluded (`grep -rn "[^n]committed baseline"` → already 0 today), 0 edits, no sentence touched. **(b) Substitute** → *"an uncommitted contract baseline"*: +1 word in `conventions.md` (Phase A's §3) and +1 in `plan.md`, which sits at 1,195/1,200 and which AC-2's third bullet forbids growing — and the literal grep still returns 2, because the substring survives. **(c)** Leave both and report in RF §6, gate recorded as 2 rather than 0 | **(a) — out of mandate.** Reproduced both hits myself before ruling: `conventions.md`:75 and `plan.md`:60, and `grep -rnE "[^n]committed baseline"` returns **0**. You are right and the TS was wrong. `uncommitted baseline` names the **absence** of the thing — it is not a synonym, and substituting into it produces a sentence that says the opposite of what it means. Option (b) also fails its own test twice: the literal grep still returns 2 because the substring survives, and it spends a word `plan.md` does not have. **AC-2's table is corrected to two names, not three** — and so is what I told the owner, who heard "three names, and `conventions.md` uses all three". `conventions.md` uses two: `frozen baseline` once and `Contract Baseline` twice, one of those a heading that already defines the canonical term. The drift is real and smaller than I stated |
| 2 | **The only genuine bare-capital `North Star` is the HL template's header field label.** Full audit of `.tfw/`: 8 literal occurrences, of which 5 are inside `Project North Star` and 1 (`glossary.md`:236) is a line-wrap of `Project North\nStar` — not drift. The two real ones are both Phase C's: `templates/HL.md`:18 `> **North Star**: {…}` (the field itself) and `:20` `> **North Star field** —` (its explainer). Nothing outside `templates/HL.md` cites the label, so a rename is local. AC-2 says capitalised `North Star` without `Project` **is** the drift; TS §9 says D touches *"the header's recovery-command line (TD-164) and nothing else in the contract block"* — the field sits below that block, so it is arguably outside the restriction, but it is Phase C's shipped surface. Which? **(a) Rename to `Project North Star` (recommended)** — 2 lines, +2 words in `templates/HL.md`, no other file affected; the field label is exactly where a reader learns the term, which is D28's whole argument. **(b) Leave** — declare a form field label a form identifier rather than running prose, record the exemption in AC-2's before/after table; 0 edits, but DoD-31 reads as unmet on a literal grep | **(a) — rename both lines.** Verified the audit: outside `Project North Star` the only live occurrences are `templates/HL.md`:18 and :20, plus the `glossary.md`:236 line-wrap you correctly excluded. Three reasons. **The field label is where a reader meets the term** — a form field that says `North Star` while the glossary defines `Project North Star` teaches the wrong name at the one moment the reader is learning it, which is D28's entire argument and this phase's mandate. **Option (b) manufactures a permanent exemption**: a rule with a documented carve-out that a grep keeps flagging is the shape of S32 — a label that resolves and quietly means something else. **Your scope reading is right and I am confirming it:** the field sits *below* the contract block, so TS §9's *"the header's recovery-command line and nothing else in the contract block"* does not reach it. TS §4 now names `templates/HL.md`'s north-star field explicitly so the extension is recorded, not inferred. Cost: 2 lines, +2 words, in a template off the F2 budget |
| 3 | **TD-158 is routed to Phase D by `TECH_DEBT.md` but appears in neither of the TS's two lists.** Its row reads *"Backlog — → Phase D terminology pass, alongside TD-140's §12 half"*. The TS names five debts in scope (157, 163, 164, 165, 167) and six re-routed out (120, 140, 142, 153, 154, 155); TD-158 is in neither, and DoF makes fixing a re-routed debt a failure while saying nothing about this one. Its content — `templates/HL.md` §3.1 bullet 2 lists *"a narrative timeline"* in the same bullet that ends *"Prose alone is not a rendering."* — can only be fixed by rewriting a sentence in Phase A's section, which HL §7.1 forbids this phase and which TD-140 already records as needing a §12 amendment first. Which? **(a) Re-route out (recommended)** — joins TD-140/141/142/153/154/155 in the follow-up task, TECH_DEBT row re-pointed, RF §6 states the reasoning; 0 risk. **(b) Fix here** — triggers DoF-1 and crosses the §7.1 section-ownership line | **(a) — re-route out.** My omission: I built the re-route list from the debts the Phase C review named and never re-read `TECH_DEBT.md` for older rows pointing at Phase D. TD-158 fails the same two tests as TD-155 — its fix is a sentence rewrite (DoF-1) inside Phase A's section (§7.1) — so it belongs in the follow-up task with TD-140, TD-153, TD-154 and TD-155. Re-point the row, state the reasoning in RF §6. **And run the check I did not:** if any other `TECH_DEBT.md` row points at Phase D and appears in neither TS list, treat it the same way and report it. A re-route list built from one review is not a re-route list |
...
> **Coordinator response to §4, §5 and §6, 2026-08-14.**
>
> **First, the part that matters more than the answers.** Six of your §6 findings are corrections to measurements I put in the TS: the parent heading, the `committed baseline` count, the `contract baseline` count, the file count, the bare `North Star` count, and the median. I re-ran the three that change what you do and **you are right on all three**. This is the second consecutive phase where the ONB corrects the coordinator's arithmetic — Phase B caught AC-6's budget. I am recording it as a pattern in the RF-facing note rather than quietly fixing the numbers, because a TS whose figures need re-deriving at ONB is costing an execution stage to do planning work.
>
> **R1 — one `##` grouping, name approved.** `## Contract and Purpose Defence`, after `## Artifact Types`, nine `###` entries in DoD-30's order. Your argument is the right one: the five contract terms and the four review-side terms are one mechanism read from two ends, and 35 → 36 groupings is the minimal structural change. Not split.
>
> **R2 — approved, and it is my error, not an ambiguity.** `Value Flow` and `Findings Map` live under `## Knowledge Terms`. I read the heading list and the term lines in two separate greps and never crossed the line ranges. AC-1's operative words are *"beside the existing siblings"*; the parenthetical is wrong and is corrected in the TS.
>
> **R3 — approved.** Edit the board row, stage nothing in `README.md`, say so in the RF. I land it.
>
> **R4 — approved, and this is the sharpest item in the file.** Folding *"12 drifted copies … not yet repaired"* verbatim into `1.2.0` would ship a release note that is false about the release it announces, on the day it ships, in the count the release corrects. Fold both `[Unreleased]` bullets into `### Changed` and correct the sentence to the repaired state, naming 14.
>
> **R5 — approved.** Close the glossary half of TD-163 only. The `plan.md` Step 2 duplication is content deletion in Phase B's file, and this phase may not do it — but report the ~50 words in RF §6 with the measurement, because it is the cheapest cut available to a file at 1,195 of 1,200 and TFW-57 needs exactly that kind of concrete candidate.
>
> **R6 — overruled, and I own it as an extension.** Add the two missing rows (`/tfw-knowledge`, `/tfw-config`) to `CLAUDE.md`'s command table. Your restraint was the right default and I am deliberately setting it aside: `CLAUDE.md` is the entry point a Claude session actually reads, `/tfw-knowledge` is a mandatory KNW-stage workflow, and omitting it makes the workflow invisible to the surface it serves — that is a live defect, not untidiness, and the fix is smaller than the report about it. **DoD-32 does not cover this** — those two workflows were not changed by this task — so it is a coordinator scope extension, recorded in TS §4 with its limit: *two rows, no other change to `CLAUDE.md`*. Not inferred, not absorbed.
>
> **§5.4 — approved, and file it as one debt, not four.** Your finding is bigger than TD-165: `gen_docs.py` resolves only `TD-{N}` and `D{N}`, so `P{N}` and `F{N}` are already aspirational in the same line. Adding `NS{N}` and `PP{N}` beside them keeps the grain honest rather than inventing a claim. One new debt covering all four patterns, naming the resolver as the site. Extending `gen_docs.py` stays out of scope.
>
> **§5.1 — recorded as evidence, not as risk.** *"No copy holds content its source lacks"* across all fourteen pairs is the finding that makes AC-3 safe, and it is the check AC-3 asks for. Put the per-pair direction result in the EV file.
>
> **§5.2 — record it.** This session being invoked from a stale `tfw-handoff` copy is TD-157 observed live on a **second** workflow, by a **second** role. It is the strongest single argument for the owner's all-fourteen decision and it arrived by accident.
>
> **§6.3, §6.4, §6.5 — all approved.** The `judge.md`:34 hybrid *"committed frozen baseline"* is a fourth form and goes in the before/after table by name. Hold articles at **p75 = 50 words**; report per-article `wc -w`. **AC-1's gate becomes case-insensitive** — `Deferral confession` ships capitalised in `judge.md`:50 and a case-sensitive `grep -c` would fail a term that exists.
