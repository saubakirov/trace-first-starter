...
12. **Only my own paths were staged, and the README hunk was checked before staging** (§7 FC1). TFW-56 executed concurrently in the same working tree and held `README.md`, `conventions.md`, `glossary.md`, `TECH_DEBT.md` and ten review-side files modified at the same time. `git add -A` at any point would have swept its work into a commit whose subject reads `[claude-code/TFW-53/phase-b/executor]`. The board-row commit was held until `git diff -U0 README.md` showed a single hunk at line 301 and nothing else.
...
| 6 | `.tfw/templates/HL.md` | §3.1, 322 words | duplication | Raised mid-execution and independently confirmed: *"Imagine it's done"* (pre-existing block) is negated 200 words later by *"A description of the plan that will produce it does not satisfy §3.1"* — 14 words spent un-saying 3. This is TD-140's measurement reached from a second direction. The fix is a replacement, and AC-11's append-not-rewrite mandate makes it a §12 amendment, not an edit. Analysis in §8 S2 |
