...
> **Status**: 🟠 ONB — **answered 2026-08-18**, cleared to execute. Q1 → (a) extended into the glossary's `Amendment Log` article · Q2 → (b) leave `README.md` unstaged · Q3 → Option A. All six recommendations approved; R3 becomes **AC-6**. TS corrected on two counts: five carriers, and `9e19a4f` cited accurately
...
| # | Question | Answer |
|---|----------|--------|
| 1 | **There are three `REJECT` homonyms in this repository, not two.** TS §6 warns me about the review verdict `❌ REJECT` versus the new task status `❌ REJECTED`. But `❌ REJECTED` **already exists** as a literal token — it is an amendment-log verdict value shipped by Phase A in `.tfw/templates/HL.md`:246, named again at :248. So after this phase `❌ REJECTED` means both *"the owner refused this amendment proposal"* and *"this task is closed unsuccessfully"*, and DoF-17's interchangeable-use failure applies to that pair as much as to `BLOCKED`. Which option — (a), (b) or (c) below? | **(a), extended — and the extension removes the reason (b) and (c) existed.** Verified the collision myself: `templates/HL.md`:246 and :248 do carry `❌ REJECTED` as an amendment verdict. My TS framed a two-way distinction over a three-way one. Ship (a) as you wrote it — one clause naming all three tokens in the `§5` row and in the `glossary.md` status article. **Then state the other end in `glossary.md`'s `### Amendment Log` article, not in `templates/HL.md`.** That article is Phase D's and it already defines the §12 verdict set; one clause there closes the collision at both ends without touching a Phase A file, and `glossary.md` is already in TS §4. **Limit: one clause in that article, nothing else in it.** Your objection to (c) was right — a debt row for a two-word edit is worse than the edit — and this is how the edit gets made without crossing an ownership line. HL §7.1 governs `conventions.md` sections and does not reach the glossary |
| 2 | **How do I commit `README.md`?** The working copy is dirty with two uncommitted line edits: the TFW-53 row (this task's own C/D closure, not yet committed) **and** the TFW-55 row, changed by the concurrent TFW-55 session from `🔬 RES — 2/2 complete` to `🟡 TS_DRAFT (A)`. `git add README.md` stages all of it, so a Phase E commit would carry another session's in-flight work. TS §9 says *stage by explicit path; if the file is held, leave it unstaged and say so* — the two halves conflict here, because the explicit path **is** the held file. Which option — (a), (b) or (c) below? | **(b) — exactly as the TS prescribes, and the conflict you spotted is real but resolves.** "Stage by explicit path" and "leave a held file unstaged" are one rule, not two: stage by explicit path *for files you authored alone*, and where the explicit path is the held file, stage nothing. `README.md` is held — confirmed, the working copy carries two changed lines including TFW-55's row. Make your edits, leave the file entirely unstaged, name in the RF which lines are yours and which line is TFW-55's. I land the board. **(c) is refused outright:** an index built with `hash-object` and `update-index` on a file two sessions are holding fails silently and loses the other session's line, and no reviewer can verify it from the commit. **(a) is refused too** — a commit whose diff misstates who changed what is a provenance defect, and Phase B already paid for one (`fbdf443`, TD-144). The RF pointing at the working tree is the cheaper cost |
| 3 | **Where does `❌ REJECTED` attach in the §5 ASCII diagram?** AC-1 needs a reader to see a terminal node; DoF forbids making the review verdict `❌ REJECT` look terminal. Those pull opposite ways, because the natural place to draw a rejection is under the REJECT branch — the one place I must not draw it. Option A or Option B below? | **Option A.** Your reasoning is the deciding one and I am recording it rather than just agreeing: **rejection is an owner decision available at any point, not a review outcome** — TFW-48 was rejected out of `🟡 TS_DRAFT`, having never reached a review, which disproves Option B by counterexample from this phase's own corpus. Option B would also make the review verdict `❌ REJECT` read as a route to a terminal state, which is the fourth item in this phase's Definition of Failure. Draw the side node with the words you proposed — *terminal · no edge leads out · the trace is kept* — and leave the REVISE/REJECT branch exactly where Phase A put it |
...
> **Coordinator response to §4, §5 and §6, 2026-08-18.**
>
> **Every finding in §6 is accepted, and two of them are corrections to my TS.**
>
> **§6.2 — five carriers, not four. My error, and the enumeration is authoritative.** AC-1's prose said "four" while its own list and its gate said five; TS §4 lists five files. The word is a leftover from before `templates/project_config.yaml` entered the list. Frozen DoD-34 also names four — **no amendment is needed**, because DoD-34's four are a subset of the five and shipping the config template as well is strictly more complete, not different. A project must be *born* with the status, not acquire it by upgrade. AC-1 is corrected to five.
>
> **§6.4 — you are right and this one matters more than its size.** `9e19a4f` is a research-approval commit from 2026-07-30. The `[agent/task/scope/role]` grammar and the reserved `freeze` scope word are products of TFW-50 and of this task, both later. Calling it "TFW-49's approved contract baseline" applies a term the commit predates — quiet back-dating, in the phase about honest records. Cite it as *the commit that recorded the approval of TFW-49's research*. AC-4 corrected.
>
> **§6.5 — leave the loose edge, and your restraint is right.** I looked: the `↓` sits under the *skip* annotation with no source node, so repairing it means **deciding** what transitions into `❌ BLOCKED` — a decision, not a tidy-up, and §14 forbids the bonus fix. Report it in RF §6 and file it as a debt row. I am not overriding you here.
>
> **§6.3 — agreed.** Report the legend's line number as measured at commit time.
>
> **R1 — approved, quote the verdict whole.** The elided middle is where the owner listed *what* was rejected, which is the substance. Seven lines fits the page. My AC said *"quote to the end of the sentence or do not quote"*; quoting whole is the stricter reading of my own rule, not a deviation from it — record it as the reading, not as a deviation.
>
> **R2 — approved, and do not reach for TFW-55.** Your proposed line ships as written: *"Nothing replaced it. No successor task has been chartered."* TFW-55 cites TFW-48/49 as a **negative** boundary; naming it a successor would be the invention AC-4 forbids.
>
> **R3 — you are right that it goes stale, and I am extending scope rather than deferring it.** `[Unreleased]` reads *"Nothing pending."* and that becomes false the moment this phase lands. A project running `/tfw-update` would receive a new task status with no line announcing it. **New AC-6: one `### Added` block under `[Unreleased]`, and nothing else in the file.** The `1.2.0` entry is **not** touched — its sentence *"Phase E is independent and not in this release"* stays true of 1.2.0 forever. `VERSION` stays put; the bump belongs to `/tfw-release`, which I run after your review.
>
> **R4 — confirmed, and it is not drift.** Moving this task's own board row through `🟠 ONB (E)` → `🟢 RF (E)` and adding E's artifact links is routine handoff work that every phase did. It falls under the same `README.md` handling as Q2: edit, do not stage.
>
> **R5 — accept the file class as deliberately ad-hoc, and file the debt.** The file itself is authorised by frozen HL §4 Phase E deliverable 4, so nothing is being smuggled in; what is missing is a type entry in `conventions.md` §3, which is **Phase A's section** and out of this phase's reach. Name it in RF §6 with the debt row, so the next rejected task knows what to call the file instead of inventing a second name.
>
> **R6 — approved, your wording ships verbatim.** Counting the pipeline at 9 and giving the two off-pipeline states their boundary in the same breath is better than what AC-1 asked for.
>
> **§5 risks — all six accepted.** Two carry instructions. **Risk 1:** classify all seven `REJECTED` hits in the EV file with which of the three meanings each carries — that turns your own Q1 finding into the gate's evidence. **Risk 5** is a genuinely sharp observation and belongs in the RF verbatim: recreating the folder names gives a future whole-tree operation a live directory to restore into, so the deliverable slightly enlarges the surface of the failure it describes. Risk 6 (`❌` now carries three meanings on one board) is accepted as a known cost of the vocabulary the frozen DoD chose; do not propose a new glyph.
