...
| Q2 | What anti-self-deception rules should conventions.md §14 include? | Open | Candidates from AFD RUNBOOK: "VERIFIED without artifact = violation," "Can't verify = BLOCKED, never assumed VERIFIED," "N/A must be justified in Evidence Plan." Defer specifics to iter2 or TS |
...
| H3 | Existing user projects contain implicit evidence patterns | open | ✅ confirmed | 4 projects scanned (Gather G3): AFD has mature pattern, helpdesk has ad-hoc pattern, AHA has honest-deferral pattern, blog has source-audit pattern. All confirm the gap between synthetic verification and real evidence |
...
|---|----------|-----------|--------|------------|
| FC1 | process | AFD testing/ system uses a 6-status vocabulary (PASS/FAIL/XFAIL/XPASS/BLOCKED/SKIP) with STATUS.md ledger per run and evidence/ folders — the most mature evidence pattern across user's projects | Subagent scan, AFD testing/README.md | High |
| FC2 | process | Helpdesk HD-28 found 2 bugs only via live testing (MissingGreenlet on lazy ORM, CORS expose_headers invisible to curl) — proving synthetic verification misses a class of real-environment issues | Subagent scan, HD-28 RF §9.2, §9.10 | High |
...
|---|--------|---------------|-----------------|
| 1 | H2: Can coordinators reliably write Evidence Plans? | If they can't, evidence planning might shift to executor autonomy | Test against real TS examples — what would an Evidence Plan look like for AHA-6, HD-28, TFW-36? |
...
| 3 | Anti-self-deception rules for conventions.md §14 | Without enforcement, agents will mark VERIFIED without real artifacts | Draft specific anti-patterns from AFD RUNBOOK §3 adapted for TFW |
...
## Conclusion
Iteration 1 established the core Evidence design through external research across 6 disciplines and internal scanning of 4 user projects (AFD, AHA, helpdesk, blog). The key decisions: "Evidence" is the right term (validated against Proof, Attestation, Acceptance via D28 naming analysis), 4-status vocabulary (VERIFIED/DEFERRED/BLOCKED/N/A) is appropriate because TFW Evidence ≠ testing system, proportional scope beats universal/mode-based, separate §5 Evidence is better than merged §4.2 (different cognitive modes), and Evidence fields in TS §5 AC items create a clean Gate/Evidence parallel. The most valuable discovery was the **testing-vs-evidence distinction**: AFD's 6-status vocabulary is designed for testing potentially-broken features, while TFW Evidence verifies completed work — a fundamentally different purpose that eliminates 3 statuses (FAIL, XFAIL, XPASS) from consideration. Without this research, the team might have transplanted AFD's vocabulary wholesale, producing a testing system when what's needed is a verification layer.
