...
| D3: Status Vocabulary | VERIFIED / DEFERRED / BLOCKED / N/A (HL proposal) | PASS / FAIL / XFAIL / XPASS / BLOCKED / SKIP (testing project system) | ✅ / ❌ / ⚠️ / 🟡 (organic per-project) | — |
...
Subagent scanned 4 real-world projects. Cross-project comparison:

| Dimension | Mobile testing project | Multi-service project | Backend API task | Blog TFW-36 |
|-----------|-------------|-------|----------------|-------------|
...
| **Mocked→real gap** | ✅ Explicit two-context (Local vs Live Beta) | 🔴 10 ACs harness-only, 1 deferred | 🔴 A lazy-loading ORM bug + a CORS header configuration issue only found live | 🔴 Fabricated citation passed all gates |
...
**Mobile testing project — the mature model:**
...
- 4 anti-self-deception rules (a project runbook): assert observable outcome, empty body ≠ PASS, known-broken ≠ PASS, can't verify = BLOCKED
...
**A multi-service project — the honest deferral model:**
...
**A backend API task — the accidental evidence model:**
...
- Two critical bugs caught ONLY by live testing: a lazy-loading ORM bug (lazy ORM vs cross-process) and a CORS header configuration issue (browser-only, invisible to curl)
- A follow-up task had ad-hoc `qa_evidence/` folder with 1 browser screenshot
...
| **A**ccurate | True representation | Anti-self-deception rules (a project runbook pattern) |
...
| Mobile testing project = mature evidence system with transferable patterns | Which elements are project-specific vs generalizable |
