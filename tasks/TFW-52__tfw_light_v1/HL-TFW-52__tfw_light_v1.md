...
**Отложенная работа, зафиксированная как долг.** Пользовательский слой всех трёх пакетов — Light, Assisted и `innoforce_starter` — написан по-русски, поскольку делался под курс. Локализация и приведение к языку остального репозитория выделены в `TECH_DEBT.md` вместе с непроверенным рантаймом hooks и семью отложенными полосами проверки. Ни один из этих пунктов не считается закрытым фактом закрытия задачи.
