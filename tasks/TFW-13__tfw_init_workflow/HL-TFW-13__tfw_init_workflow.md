...
- Пользователь видит TFW в действии с первого момента
- Task Board не пустой — сразу есть живой пример
- Процесс обучает: агент объясняет что делает и зачем ("Я создаю Task Board — это центральное место для всех задач")
- RF от init = документация о том, как был настроен проект
...
- ✅ 5. Init автоматически создаёт `{PREFIX}-1` на Task Board и закрывает его
- ✅ 6. Старый `init.md` удалён или заменён указателем
- ✅ 7. "How to Write a New Adapter" перенесён в адекватное место
- ✅ 8. Все ссылки на init.md обновлены
