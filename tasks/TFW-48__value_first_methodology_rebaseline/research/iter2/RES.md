...
```mermaid
flowchart TD
  P["Product purpose + applicable project values"] --> M5["M5 leading challenged candidate"]
  M5 --> R["Rule records: observable locality + full local R9 gates"]
  M5 --> L["Learning: event entry + disposition-typed receipt"]
  M5 --> E["Extensions: independent registered lifecycle + observable freshness/conflict"]
  M5 --> V["Proof records: local always; seam/live/debt when triggered"]
  R --> K["K3 semantic obligations"]
  L --> K
  E --> K
  V --> K
  N["Numeric inventory"] --> S["Type + owner + consumer + count + response + override"]
  S --> O["Restore owner/consumer or retire normativity"]
  O --> C["Calibrate only after semantics"]
  H4["H4 unresolved"] --> T0["T0 desk protocol authorized"]
  H4 --> UX["40 / 112 / 220 unapproved owner options"]
```
